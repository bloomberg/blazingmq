...
void EventHandler::onSessionEvent(const bmqa::SessionEvent& sessionEvent)
{
    ...
    case bmqt::SessionEventType::e_CONNECTION_TIMEOUT: {
        ...
        BALL_LOG_ERROR << "Connection to the broker has timed out. "
                       << "Please check if BlazingMQ broker is running on "
                       << "your machine";
    } break;
...
static void consume(bmqa::Session* session, QueueManager* queueManager)
{
    ...
    // Note that 'openQueueSync()' takes another very useful optional parameter
    // bmqt::QueueOptions. For the sake of simplicity of this tutorial, we have
    // not passed this as a parameter. Sample usage is given below:
    //
    // bmqt::QueueOptions queueOptions;
    //
    // const int k_PRIORITY = 10;  // Default priority is '0'
    // queueOptions.setConsumerPriority(k_PRIORITY);
    //
    // Note that setting consumer priority is very useful in scenarios where
    // client applications have multiple consumers for the same queue or
    // Application ID (as in the case of Fanout queue).
    //
    // const int k_MAX_UNCONFIRMED_MESSAGES = 1000; // Default is 1000 too
    // queueOptions.setMaxUnconfirmedMessages(k_MAX_UNCONFIRMED_MESSAGES);
    //
    // const int k_MAX_UNCONFIRMED_BYTES = 33554432; // Default is 33554432 too
    // queueOptions.setMaxUnconfirmedBytes(k_MAX_UNCONFIRMED_BYTES);
    //
    // Note that setting maximum unconfirmed messages and bytes is useful to
    // give clients control so that they can prevent consumer applications from
    // getting overwhelmed with new messages (also see the usage of
    // 'configureQueueSync' API in this tutorial).
    //
    // Refer to documentation of bmqt::QueueOptions,
    // bmqa::Session::openQueueSync() for further details.
