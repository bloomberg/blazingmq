                      ...
                      QueueManager*        queueManager)
{
    ...
    // We will associated some properties with the published message.  In order
    ...
    // Cycle through property values, let each consumer receive each message
    // using appropriate subscription.
    ...
    rc = properties.setPropertyAsString("firmId", data.first);
    if (rc != 0) {
        // Failed to set a property.  This may or may not be a fatal error for
        // an application.  In this case, we treat it as non-fatal.

        BALL_LOG_ERROR << "Failed to set 'firmId' property on the message, "
                       << "rc: " << rc;
    }

    rc = properties.setPropertyAsInt32("price", data.second);
    if (rc != 0) {
        // Failed to set a property.  This may or may not be a fatal error for
        // an application.  In this case, we treat it as non-fatal.

        BALL_LOG_ERROR << "Failed to set 'price' property on the message, "
                       << "rc: " << rc;
    }
