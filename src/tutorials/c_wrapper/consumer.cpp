...
void onMessageEvent(const z_bmqa_MessageEvent* messageEvent)
...
static void consume(z_bmqa_Session* session)
{
    ...
    z_bmqa_Session__openQueueSync(session, queueId, k_QUEUE_URL, static_cast<uint64_t>(bmqt::QueueFlags::e_READ), &status);

    if (!z_bmqa_OpenQueueStatus__toBool(status) || !z_bmqa_QueueId__isValid(queueId)) {
    ...
    z_bmqa_Session__configureQueueSync(session, queueId, options, 0, &configureStatus);
