...
/// |---------------------+------------------------------------------+---------------------------|
/// | connectClient       | This role can connect as a client.       | None                      |
/// | connectAdmin        | This role can connect as an admin.       | None                      |
/// | connectClusterNode  | This role can connect as a cluster node. | None                      |
/// | queueRead           | This role can open queues for reading.   | `{ "id": queueResource }` |
/// | queueWrite          | This role can open queues for writing.   | `{ "id": queueResource }` |
/// | executeAdminCommand | This role can execute admin commands.    | None                      |
// clang-format on
///
...
class Policy_Permission {
  ...
  public:
    ...
    bool isConnectProxyAllowed() const;

    bool isConnectAdminAllowed() const;

    bool isConnectClusterNodeAllowed() const;
