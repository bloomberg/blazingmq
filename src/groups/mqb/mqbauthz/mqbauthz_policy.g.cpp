...
template <typename T>
int simpleDecode(T*                      obj,
                 bsl::string_view        encoded,
                 const bsl::allocator<>& alloc)
{
    bsl::istringstream     str(encoded);
    baljsn::Decoder        decoder(alloc.mechanism());
    baljsn::DecoderOptions decoderOptions;
    decoderOptions.setSkipUnknownElements(false);
    int rc = decoder.decode(str, obj, decoderOptions);
    if (rc != 0) {
        PVV_SAFE(decoder.loggedMessages());
    }
    return rc;
}
...
class TestPolicy_UriResource : public BaseTest {
  protected:
    struct TestCase {
        ...
        TestCase(bsl::string_view uri, bool shouldMatch)
        : d_uri(uri)
        , d_shouldMatch(shouldMatch)
        {
        }
    };
};
...
TEST_F(TestPolicy_UriResource, domainPatternMatchesAllQueuesInDomain)
{
    ...
    testCases.emplace_back("bmq://bmq-test-uri-1/q123", true);
    testCases.emplace_back("bmq://bmq-test-uri-1.~pd/q123", true);
    testCases.emplace_back("bmq://bmq-test-uri-1.~pd/q123?id=myapp123", true);
    testCases.emplace_back("bmq://bmq-test-uri-2/q123", false);
    testCases.emplace_back("bmq://BMQ-TEST-URI-1/q123", false);
    ...
    bmqt::Uri                                     uri(alloc.mechanism());
    typedef bsl::vector<TestCase>::const_iterator It;
    for (It it = bsl::cbegin(testCases), end = bsl::cend(testCases); it != end;
         ++it) {
        ASSERT_EQ(0, bmqt::UriParser::parse(&uri, NULL, it->d_uri));
        EXPECT_EQ(it->d_shouldMatch, uriResource.matches(uri));
    }
}
...
TEST_F(TestPolicy_UriResource, queuePatternMatchesQueue)
{
    ...
    testCases.emplace_back("bmq://bmq-test-uri-1.~pd/q123", true);
    testCases.emplace_back("bmq://bmq-test-uri-1.~pd/q123?id=myapp123", true);
    testCases.emplace_back("bmq://bmq-test-uri-1/q456", false);
    testCases.emplace_back("bmq://bmq-test-uri-2/q123", false);
    testCases.emplace_back("bmq://BMQ-TEST-URI-1/q123", false);
    ...
    bmqt::Uri uri(alloc.mechanism());
    for (const TestCase *it  = bsl::cbegin(testCases),
                        *end = bsl::cend(testCases);
         it != end;
         ++it) {
        ASSERT_EQ(0, bmqt::UriParser::parse(&uri, NULL, it->d_uri));
        EXPECT_EQ(it->d_shouldMatch, uriResource.matches(uri));
...
TEST_F(TestPolicy_Permission, emptyPermissionDenysAll)
{
    ...
    EXPECT_FALSE(permission.isConnectProxyAllowed());
    EXPECT_FALSE(permission.isConnectAdminAllowed());
    EXPECT_FALSE(permission.isConnectClusterNodeAllowed());
...
TEST_F(TestPolicy_Permission, adminClient)
{
    ...
    EXPECT_FALSE(permission.isConnectClientAllowed());
    EXPECT_FALSE(permission.isConnectProxyAllowed());
    EXPECT_TRUE(permission.isConnectAdminAllowed());
    EXPECT_FALSE(permission.isConnectClusterNodeAllowed());
...
TEST_F(TestPolicy_Permission, proxyBroker)
{
    ...
    EXPECT_FALSE(permission.isConnectClientAllowed());
    EXPECT_TRUE(permission.isConnectProxyAllowed());
    EXPECT_FALSE(permission.isConnectAdminAllowed());
    EXPECT_FALSE(permission.isConnectClusterNodeAllowed());
...
TEST_F(TestPolicy_Permission, clusterNode)
{
    ...
    EXPECT_FALSE(permission.isConnectProxyAllowed());
    EXPECT_FALSE(permission.isConnectAdminAllowed());
    EXPECT_TRUE(permission.isConnectClusterNodeAllowed());
...
TEST_F(TestPolicy, doctestPolicyDefinition)
{
    ...
    bsl::string_view policyJson =
        ...
        "    ]"
        "}";
    mqbpoly::Policy policyDef(alloc.mechanism());
    ASSERT_EQ(0, decodePolicyJson(&policyDef, policyJson));

    mqbauthz::Policy policy;
    EXPECT_EQ(0, mqbauthz::Policy::parse(&policy, policyDef, alloc));

    // Now check user "anonymous"'s permissions
    bsl::optional<const mqbauthz::Policy::Permission*> anonPermissionOpt =
        policy.get("anonymous");
    ASSERT_TRUE(anonPermissionOpt.has_value());
    ASSERT_TRUE(NULL != *anonPermissionOpt);

    const mqbauthz::Policy::Permission& anonPermission = **anonPermissionOpt;
    EXPECT_TRUE(anonPermission.isConnectClientAllowed());
    EXPECT_TRUE(!anonPermission.isConnectAdminAllowed());
    EXPECT_TRUE(
        anonPermission.isQueueReadAllowed("bmq://bmq-test-domain/test-queue"));
    EXPECT_TRUE(anonPermission.isQueueWriteAllowed(
        "bmq://bmq-test-domain/test-queue"));
    EXPECT_TRUE(!anonPermission.isExecuteAdminCommandAllowed("HELP"));
}

TEST_F(TestPolicy, noPermissions)
{
    bsl::allocator<> alloc = bmqtst::TestHelperUtil::allocator();
    bsl::string_view policyJson =
        "{"
    ...
    const mqbauthz::Policy::Permission& anonPermission = **anonPermissionOpt;
    EXPECT_FALSE(anonPermission.isConnectClientAllowed());
    EXPECT_FALSE(anonPermission.isConnectAdminAllowed());
    EXPECT_FALSE(
        anonPermission.isQueueReadAllowed("bmq://bmq-test-domain/test-queue"));
    EXPECT_FALSE(anonPermission.isQueueWriteAllowed(
        "bmq://bmq-test-domain/test-queue"));
    EXPECT_FALSE(anonPermission.isExecuteAdminCommandAllowed("HELP"));
}

TEST_F(TestPolicy, getNonExistantRole)
{
    bsl::allocator<> alloc      = bmqtst::TestHelperUtil::allocator();
    bsl::string_view policyJson = "{"
                                  "    \"roles\": []"
    ...
    bsl::optional<const mqbauthz::Policy::Permission*> anonPermissionOpt =
        policy.get("anonymous");
    ASSERT_FALSE(anonPermissionOpt.has_value());
}

TEST_F(TestPolicy, rolesMustBeUnique)
{
    bsl::allocator<> alloc = bmqtst::TestHelperUtil::allocator();
    bsl::string_view policyJson =
        "{"
    ...
    mqbauthz::Policy policy;
    EXPECT_NE(0, mqbauthz::Policy::parse(&policy, policyDef, alloc));
}
