...
void StorageManager::do_replicaDataRequestPush(const EventWithData& event)
{
    ...
    if (eventType == PartitionFSM::Event::e_REPLICA_STATE_RSPN ||
        ...
        // Self primary must have triggered this event, either by certifying
        // self as highest-sequence-number node, or by receiving a
        // `ReplicaDataRequestPull` from the up-to-date replica.
        BSLS_ASSERT_SAFE(
            eventType == PartitionFSM::Event::e_SELF_HIGHEST_SEQ ||
            eventType == PartitionFSM::Event::e_REPLICA_DATA_RQST_PULL);
        BSLS_ASSERT_SAFE(eventData.source()->nodeId() == selfNode->nodeId());
...
void StorageManager::do_replicaDataRequestDrop(const EventWithData& event)
{
    ...
    if (eventType == PartitionFSM::Event::e_REPLICA_STATE_RSPN ||
        ...
        // receiving a ReplicaDataRequestPull from the up-to-date replica.
