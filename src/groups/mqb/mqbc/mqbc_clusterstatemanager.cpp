...
void ClusterStateManager::do_sendCSLPatch(const EventWithMetadata& event)
{
    ...
    const ClusterFSMEventMetadata&                metadata = event.second;
    const ClusterFSMEventMetadata::InputMessages& inputMessages =
        metadata.inputMessages();
    for (ClusterFSMEventMetadata::InputMessagesCIter it =
             inputMessages.cbegin();
         it != inputMessages.cend();
         ++it) {
    ...
    BSLA_MAYBE_UNUSED const EventWithMetadata& event)
{
    ...
    // CSL is the source of truth for the Primary only,
    // and we assume Leader and Primary are always the same node.
