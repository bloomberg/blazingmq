...
/// How long `waitUntilApplied` sleeps between polls, and how long it polls in
/// total before giving up on a record that was accepted but never applied.
const int k_APPLY_POLL_MS    = 20;
const int k_APPLY_TIMEOUT_MS = 30 * 1000;

/// Load into the specified `isApplied` whether the specified `partitionId` of
/// the specified `provider` has applied the record at the specified
/// `sequenceNumber`, and into the specified `isLeader` whether it is still the
/// primary.  `isLeader` is read here rather than by the waiting thread because
/// it reads state this thread owns ('FileStore::d_primaryNode_p', the Raft
/// node's state); reading both in one visit also keeps them consistent with
/// each other.
///
/// THREAD: Executed by `partitionId`s dispatcher thread.
...
{
    const mqbs::RecordStore* recordStore = provider->recordStore(partitionId);

    *isApplied = recordStore->hasApplied(sequenceNumber);
    *isLeader  = recordStore->isLeader();
}

/// Set the replication factor of the specified `partitionId` of the specified
/// `provider` to the specified `value`.
///
/// THREAD: Executed by `partitionId`s dispatcher thread.
void setReplicationFactorDispatched(mqbi::StorageProvider* provider,
                                    int                    partitionId,
                                    int                    value)
{
    provider->recordStore(partitionId)->setReplicationFactor(value);
}

/// Set the availability of the specified `partitionId` of the specified
/// `provider` to the specified `isAvailable`.
///
/// THREAD: Executed by `partitionId`s dispatcher thread.
void setAvailabilityStatusDispatched(mqbi::StorageProvider* provider,
                                     int                    partitionId,
                                     bool                   isAvailable)
{
    provider->recordStore(partitionId)->setAvailabilityStatus(isAvailable);
}
                                         ...
                                         const AppInfos& appIdKeyPairs)
{
    ...
    // 'addQueueOpRecordHandle' below takes the handle of a record that has
    // only been written, not committed.  Raft registers the queue when the
    // record commits instead ('createQueueStorageAsPrimary' proposes and
    // returns), so this runs on a legacy partition only.
    BSLS_ASSERT_SAFE(!rs->isRaft());
...
void StorageUtil::executeOnPartition(mqbi::StorageProvider* provider,
                                     int                    partitionId,
                                     const mqbi::Dispatcher::VoidFunction& job)
{
    ...
    // PRECONDITIONS
    BSLS_ASSERT_SAFE(provider);

    mqbi::DispatcherClient* partition = provider->partition(partitionId);
    BSLS_ASSERT_SAFE(partition);

    partition->dispatcher()->execute(job,
                                     partition,
                                     mqbi::DispatcherEventType::e_CALLBACK);
}

void StorageUtil::synchronizePartition(mqbi::StorageProvider* provider,
                                       int                    partitionId)
{
    // executed by *ANY* thread other than 'partitionId's dispatcher one
    ...
    partition->dispatcher()->synchronize(partition);
}
                                        ...
                                        const StorageFilters&  filters)
{
    ...
    BSLS_ASSERT_SAFE(storageLists->size() >
                     static_cast<unsigned int>(partitionId));

    provider->recordStore(partitionId)
        ->getStorages(&((*storageLists)[partitionId]), filters);
                               ...
                               StorageMonitor*        storageMonitor)
{
    ...
    storageLists.resize(storageMonitor->numPartitions());
                             ...
                             bslma::Allocator*      allocator)
{
    ...
    const int numPartitions = storageMonitor->numPartitions();

    // A rollover of every partition when 'partitionId' is negative, of one
    // otherwise.
    const int first = partitionId < 0 ? 0 : partitionId;
    const int last  = partitionId < 0 ? numPartitions : partitionId + 1;

    bslmt::Latch                     latch(last - first);
    bsl::vector<int>                 rcs(numPartitions, allocator);
    bsl::vector<bsls::Types::Uint64> rolloverSeqNums(numPartitions,
                                                     0,
                                                     allocator);

    for (int i = first; i < last; ++i) {
        executeOnPartition(provider,
                           i,
    ...
    // The rollovers above are only proposed on a Raft partition; hold the
    // response until each has applied, so that a reported success means the
    // file set was actually swapped.  One deadline covers them all -- they
    // ran in parallel.
    const bsls::TimeInterval deadline =
        bmqu::Time::nowMonotonicClock().addMilliseconds(k_APPLY_TIMEOUT_MS);

    bool               allSuccess = true;
    bmqu::MemOutStream output(allocator);

    for (int i = first; i < last; ++i) {
        const int rc = rcs[i];
        if (rc != 0) {
            output << "Rollover failed on partition " << i << ", rc: " << rc
                   << ". ";
            allSuccess = false;
            continue;  // CONTINUE
        }

        if (!waitUntilApplied(&output,
                              provider,
                              i,
                              rolloverSeqNums[i],
                              "Rollover",
                                       ...
                                       int                    partitionId)
{
    ...
    BSLS_ASSERT_SAFE(latch);
    BSLS_ASSERT_SAFE(rc);
    BSLS_ASSERT_SAFE(rolloverSeqNum);
    BSLS_ASSERT_SAFE(provider);

    enum { rc_ROLLOVER_REJECTED = -1 };

    mqbs::RecordStore* recordStore = provider->recordStore(partitionId);

    *rolloverSeqNum = 0;

    if (recordStore->isTransferringLeadership()) {
        // An 'e_ROLLOVER' appended now is one more entry the target has to
        // replicate before it can be sent its 'TimeoutNow', so it works
        // against the handover this node is in the middle of.
        BALL_LOG_WARN << "Partition [" << partitionId
                      << "] Rollover rejected: this node is handing over "
                      << "leadership of the partition.";
        *rc = rc_ROLLOVER_REJECTED;
        latch->arrive();
        return;  // RETURN
    }
    ...
    *rc = recordStore->rollover();
    ...
    if (0 == *rc) {
        // On a Raft partition 'rollover' only proposes the 'e_ROLLOVER'
        // record; the file set is swapped when it commits.  Read its sequence
        // number here, on the thread that just appended it, so that nothing
        // can append past it first.
        *rolloverSeqNum = recordStore->writeHeadSeqNum();
    }
                                       ...
                                       const bsl::string&     targetName)
{
    ...
    // Whether this node leads the partition is state the partition thread
    // owns, so the checks, the transfer and the verdict all happen there.
    // Waiting is cheap: 'transferLeadership' only sends, it does not wait for
    // the target to take over.
    executeOnPartition(provider,
                       partitionId,
                       bdlf::BindUtil::bind(&doTransferLeadershipDispatched,
                                            result,
                                            provider,
                                            partitionId,
                                            targetName));
    synchronizePartition(provider, partitionId);
}
    ...
    const bsl::string&     targetHostName)
{
    ...
    if (!recordStore->isRaft()) {
        result->makeError().message() = "Transfer leadership is not supported";
        return;  // RETURN
    }

    const int rc = recordStore->transferLeadership(targetHostName);
    if (0 != rc) {
        // Reported rather than only logged: without it the caller would poll
        // for a transfer that was never started and time out instead of
        // learning why.
        bmqu::MemOutStream output;
        output << "Transfer leadership of partition " << partitionId << " to '"
               << targetHostName << "' was rejected, rc: " << rc;
        result->makeError().message() = output.str();
        return;  // RETURN
    }

    // Initiated only; the caller waits for the new primary and owns the
    // verdict.
    result->makeSuccess();
}
    ...
    const bslstl::StringRef& partitionLocation)
{
    ...
    summary.fileStores().resize(storageMonitor->numPartitions());
    ...
    mqbi::StorageProvider*         provider)
{
    ...
    provider->recordStore(partitionId)
        ->loadSummary(&summary->fileStores()[partitionId]);
                                           ...
                                           StorageMonitor* storageMonitor)
{
    ...
    bslmt::Latch latch(numPartitions);

    for (int i = 0; i < numPartitions; ++i) {
        executeOnPartition(provider, i, bdlf::BindUtil::bind(job, i, &latch));
    }
            ...
            *replicationFactor = tunable.value().theInteger();
            ...
            for (int i = 0; i < numPartitions; ++i) {
                executeOnPartition(
                    provider,
                    i,
                    bdlf::BindUtil::bind(&setReplicationFactorDispatched,
                                         provider,
                                         i,
    ...
    bslma::Allocator*            allocator)
{
    ...
    BSLS_ASSERT_SAFE(recordStore->storageMonitor());
    ...
    for (QueueKeyInfoMapConstIter qit = queueKeyInfoMap->begin();
        ...
        if (!qinfo.isRecorded()) {
            // No storage is created for this queue below, so it needs no
            // domain either.  Requesting one blocks on the domain factory,
            // and the queue may be one whose domain is gone -- deleted in a
            // part of the log this recovery does not fold in.
            continue;  // CONTINUE
        }
    ...
    for (QueueKeyInfoMapConstIter qit = queueKeyInfoMap->begin();
        ...
        if (!qinfo.isRecorded()) {
            // Either the queue is in the cluster state only, or its creation
            // is above the rollover boundary and belongs to the commit-apply
            // path.  Ahead of the uniqueness checks below, which hold for
            // recovered state alone: an unapplied delete-and-recreate leaves
            // two queueKeys for one uri, and only apply resolves which
            // survives.
            BALL_LOG_INFO << clusterDescription << " Partition ["
                          << partitionId
                          << "]: not recovered state, skips creating "
                          << "storage(s) for queueUri [" << queueUri
                          << "], queueKey [" << queueKey << "].";
        ...
        const StorageSp& rstorage = recordStore->storageMonitor()->find(
            queueUri);
        if (rstorage) {
        ...
        BSLS_ASSERT_SAFE(!recordStore->storageMonitor()->find(queueUri));

        recordStore->storageMonitor()->onStorageRegistered(partitionId,
                                                           queueUri,
                                                           rs_sp,
                                                           appIdKeyPairs);
    ...
    // Calculate offsets
    recordStore->storageMonitor()->onRecovered(partitionId);
                                         ...
                                         mqbi::Domain*           domain)
{
    ...
    const StorageSp& storageSp = rs->storageMonitor()->find(uri);
    if (storageSp) {
                                              ...
                                              mqbi::Domain*   domain)
{
    ...
    // PRECONDITIONS
    BSLS_ASSERT_SAFE(rs);
    BSLS_ASSERT_SAFE(rs->storageMonitor());
    BSLS_ASSERT_SAFE(uri.isValid());
    ...
    if (storageSp) {
        if (0 == registerQueueDispatched(rs, storageSp.get(), appIdKeyPairs)) {
            ...
            BSLS_ASSERT_SAFE(!rs->storageMonitor()->find(uri));
                                            ...
                                            const bmqt::Uri&     uri)
{
    ...
    const StorageSp& storage = rs->storageMonitor()->find(uri);
    if (!storage) {
    ...
    rs->storageMonitor()->onStorageUnregistered(
        partitionId,
    ...
    const mqbu::StorageKey& appKey)
{
    ...
    StorageSp storageSp = rs->storageMonitor()->find(uri);
    if (!storageSp) {
    ...
    if (appKey.isNull()) {
        ...
        rs->storageMonitor()->onStorageUnregistered(rs->partitionId(), uri);
    ...
    mqbi::Domain*           domain)
{
    ...
    StorageSp storage = rs->storageMonitor()->find(uri);
    if (!storage) {
    ...
    const bsl::string&                domainName)
{
    ...
    BSLS_ASSERT_SAFE(static_cast<unsigned int>(partitionId) <
                     purgedQueuesResultsVec->size());

    mqbs::RecordStore* recordStore = provider->recordStore(partitionId);

    if (leadersOnly && !recordStore->isLeader()) {
        // Report nothing for a partition this node does not lead, rather than
        // the per-queue rejection 'purgeQueueDispatched' would produce.  The
        // check belongs here because leadership is state this thread owns.
        latch->arrive();
        return;  // RETURN
    }
    ...
    storageMonitor->loadAllStorages(&allStorages, partitionId);
...
void StorageUtil::findAndPurgeQueueDispatched(
    ...
    const bsl::string&        appId)
{
    ...
    *purgeSeqNum = 0;
    ...
    // This thread is the one that registers and unregisters storages, so the
    // lookup and the purge below cannot be separated by either.  The lookup
    // that routed the command here ran on the command's thread and is only a
    // hint: by now the queue may be gone, or assigned elsewhere.
    const StorageSp storage = storageMonitor->find(uri);

    if (!storage || storage->partitionId() != partitionId) {
        bmqu::MemOutStream errorMsg;
        errorMsg << "Not purging queue '" << uri
                 << "': it is no longer registered on partition ["
                 << partitionId << "]";
    ...
    const bsl::string&        appId)
{
    ...
    // for 'waitUntilApplied' to wait on.
    *purgeSeqNum = 0;
    ...
    if (recordStore->isTransferringLeadership()) {
        // The purge record would be held rather than written, so
        // 'purgeSeqNum' would name an earlier record that has already
        // applied and this command would report a success that never
        // happened.  Rejected rather than held: the caller is waiting.
        bmqu::MemOutStream errorMsg;
        errorMsg << "Not purging queue '" << storage->queueUri()
                 << "' with record store: " << recordStore->description()
                 << " [reason: this node is handing over leadership of the "
                 << "partition]";
        mqbcmd::Error& error = purgedQueueResult->makeError();
        error.message()      = errorMsg.str();
        return;  // RETURN
    ...
    // 'waitUntilApplied' holds the response until this sequence number
    // applies, so by the time the caller sees them they are what was.
...
bool StorageUtil::waitUntilApplied(bmqu::MemOutStream*       errorDescription,
                                   mqbi::StorageProvider*    provider,
                                   int                       partitionId,
                                   bsls::Types::Uint64       sequenceNumber,
                                   ...
                                   const bsls::TimeInterval& deadline)
{
    ...
    for (;;) {
        ...
        executeOnPartition(provider,
                           partitionId,
                           bdlf::BindUtil::bind(&hasAppliedDispatched,
                                                &isApplied,
                                                &isLeader,
                                                provider,
                                                partitionId,
        ...
        // A proposal applies where it was made.  Once this node stops being
        ...
        if (!isLeader || bmqu::Time::nowMonotonicClock() >= deadline) {
            *errorDescription
                << operation << " on "
                << provider->partition(partitionId)->description()
        ...
        bslmt::ThreadUtil::microSleep(k_APPLY_POLL_MS * 1000);
    }
}
...
void StorageUtil::processCommand(mqbcmd::StorageResult*     result,
                                 ...
                                 bslma::Allocator*        allocator)
{
    // executed by the thread issuing the command (admin).
    ...
    if (command.isSummaryValue()) {
        ...
        if (partitionId >= storageMonitor->numPartitions()) {
        ...
        executeOnPartition(
            provider,
            partitionId,
            bdlf::BindUtil::bind(&setAvailabilityStatusDispatched,
                                 provider,
                                 partitionId,
                                 isAvailable));
        ...
        if (command.domain().command().isQueueStatusValue()) {
            ...
            bsl::vector<bsl::vector<mqbcmd::PurgeQueueResult> >
                purgedQueuesVec;
            purgedQueuesVec.resize(numPartitions);

            bsl::vector<bsls::Types::Uint64> purgeSeqNums(numPartitions,
                                                          0,
                                                          allocator);
            ...
            for (int i = 0; i < numPartitions; ++i) {
                if (!waitUntilApplied(&errorDescription,
                                      provider,
                                      i,
                                      purgeSeqNums[i],
                                      "Purge",
        ...
        // One lookup covers every partition: the monitor is cluster-wide.
        // This one only routes the purge to a partition thread; the storage
        // it is to act on is looked up again there, by
        // 'findAndPurgeQueueDispatched'.
        const StorageSp queueStorage = storageMonitor->find(uri);
        ...
        executeOnPartition(provider,
                           partitionId,
                           bdlf::BindUtil::bind(&findAndPurgeQueueDispatched,
                                                &purgedQueueResult,
                                                &purgeSeqNum,
                                                provider,
                                                storageMonitor,
                                     ...
                                     StorageMonitor*        storageMonitor)
{
    ...
    // dispatcher.
    bsl::vector<bsls::Types::Uint64> purgeSeqNums(numPartitions, 0);

    // To purge a domain, we have to purge queues in each partition
    // where the current node is the primary
    // from the correct thread.  This is achieved by parallel launch
    // of `purgeDomainDispatched` across all record stores' threads, each
    // returning right away if this node does not lead its partition.
    // We need to wait here, using `latch`, until the command completes
    // in all threads.
    executeForEachPartitions(
        bdlf::BindUtil::bind(&purgeDomainDispatched,
...
// CREATORS
StorageMonitor::StorageMonitor(mqbi::Cluster*    cluster,
                               bslma::Allocator* allocator)
...
void StorageMonitor::resize(int numPartitions)
{
...
// MANIPULATORS
void StorageMonitor::onStorageRegistered(
    int                                             partitionId,
    const bmqt::Uri&                                uri,
                                         ...
                                         const mqbi::Storage::AppInfos& apps)
{
    ...
    BALL_LOG_WARN << "storageMonitor registering storage for queue [" << uri
...
void StorageMonitor::onStorageUnregistered(int              partitionId,
                                           const bmqt::Uri& uri)
{
    BSLS_ASSERT_SAFE(0 <= partitionId &&
                     partitionId < static_cast<int>(d_storages.size()));

    BALL_LOG_WARN << "storageMonitor unregistering storage for " << uri;
...
void StorageMonitor::releaseStorages(int partitionId)
{
    // No notification: for use during teardown, when the owning cluster may
    // already be stopped/partially destroyed.
    bslmt::LockGuard<bslmt::Mutex> guard(
        d_storageLockVec[partitionId].get());  // LOCK
...
void StorageMonitor::onRecovered(int partitionId)
{
    BSLS_ASSERT_SAFE(0 <= partitionId &&
                     partitionId < static_cast<int>(d_storages.size()));
...
void StorageMonitor::loadAllStorages(bsl::vector<StorageSp>* result,
                                     int                     partitionId)
{
    BSLS_ASSERT_SAFE(result);
    BSLS_ASSERT_SAFE(0 <= partitionId &&
...
bool StorageMonitor::isStorageEmpty(const bmqt::Uri& uri,
...
bool StorageMonitor::isRaft() const
{
    return false;
}
