...
// NOLINTBEGIN(cppcoreguidelines-explicit-virtual-functions,cppcoreguidelines-special-member-functions)
...
// NOLINTEND(cppcoreguidelines-explicit-virtual-functions,cppcoreguidelines-special-member-functions)
...
// NOLINTBEGIN(*-magic-numbers,cppcoreguidelines-use-enum-class)
{
    ...
    // The snapshot will have the same LSN as the record which
...
// NOLINTBEGIN(*-magic-numbers,cppcoreguidelines-use-enum-class)
{
    ...
    if (lsn < d_clusterData_p->electorInfo().leaderMessageSequence()) {
        BALL_LOG_WARN
            << description()
            << ": Failed to apply advisory: " << clusterMessage
            << ". Reason: advisory is stale (LSN: " << printLSN(lsn)
            << ", self LSN: "
    ...
    if (d_uncommittedAdvisories.find(lsn) != d_uncommittedAdvisories.end()) {
    ...
    case (ClusterStateRecordType::e_UPDATE): {
        if (isSelfLeader()) {
            ...
            BALL_LOG_ERROR << description()
                           ...
                           << printLSN(lsn) << ") is less than self LSN ("
                           << printLSN(d_clusterData_p->electorInfo()
...
// NOLINTBEGIN(*-magic-numbers,cppcoreguidelines-use-enum-class)
{
    ...
    commitAdvisory.sequenceNumberCommitted() = lsn;
    ...
    BALL_LOG_INFO << description() << " Quorum of " << ackQuorum
                  << " acks is achieved for advisory of LSN " << printLSN(lsn)
...
void IncoreClusterStateLedger::cancelUncommittedAdvisories()
{
    ...
    if (iter != d_uncommittedAdvisories.end()) {
        const bmqp_ctrlmsg::LeaderMessageSequence& lsn = iter->first;
    ...
    if (isSelfLeader()) {
        ...
        for (; iter != d_uncommittedAdvisories.cend(); ++iter) {
            ...
            if (info.d_ackCount >= ackQuorum) {
                ...
                // iterator will be invalidated. So we just store the LSNs
                // in the vector and run over it again.
...
// NOLINTBEGIN(*-magic-numbers,*-narrowing-conversions,cppcoreguidelines-use-enum-class)
{
    ...
    // Validate LSN and record type
    bmqp_ctrlmsg::LeaderMessageSequence lsn;
    lsn.electorTerm()    = recordHeader->electorTerm();
    lsn.sequenceNumber() = recordHeader->sequenceNumber();
    ...
    if (isSelfLeader()) {
        ...
        if (recordHeader->recordType() != ClusterStateRecordType::e_ACK) {
            BALL_LOG_ERROR
                ...
                << printLSN(lsn) << " from '" << source->nodeDescription()
        ...
        if (d_uncommittedAdvisories.find(lsn) !=
            d_uncommittedAdvisories.end()) {
            BALL_LOG_ERROR << description()
                           << ": Failed to apply record with LSN = "
                           << printLSN(lsn) << " from '"
                           << source->nodeDescription()
        ...
        if (lsn < d_clusterData_p->electorInfo().leaderMessageSequence()) {
            BALL_LOG_ERROR
                << description()
                << ": Failed to apply record with LSN = " << printLSN(lsn)
    ...
    if (rc != 0) {
        BALL_LOG_WARN
            ...
            << source->nodeDescription() << "' [LSN: " << printLSN(lsn)
            << ", self LSN: "
            << printLSN(d_clusterData_p->electorInfo().leaderMessageSequence())
            << "]. rc: " << rc;
