                                    ...
                                    bool                    forceUpdate)
{
    ...
    if (domIt != clusterState->domainStates().end()) {
        ...
        if (cit != domIt->second->queuesInfo().cend()) {
            ...
            mwcu::Printer<AppInfos> stateAppInfos(&qs->appInfos());
            mwcu::Printer<AppInfos> storageAppInfos(&appInfos);
            ...
            if (!forceUpdate) {
                MWCTSK_ALARMLOG_ALARM("CLUSTER_STATE")
                    ...
                    << storageAppInfos << "]." << MWCTSK_ALARMLOG_END;
                return;  // RETURN
            }
    ...
    mwcu::Printer<AppInfos> printer(&appInfos);
    BALL_LOG_INFO << cluster->description()
