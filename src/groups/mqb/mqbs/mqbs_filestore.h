...
/// The fields of a `Proposal` an `e_QUEUE_OP` creation/addition
/// record uses, and no other record type does.  Held by the proposal
...
struct QueueInfo {
    ...
    bmqt::Uri                d_queueUri;
    mqbi::Storage::AppInfos  d_appIdKeyPairs;
    bool                     d_isNewQueue;
...
/// 'formatMessageRecord'.
///
/// At namespace scope rather than nested in `FileStore` so that
/// `RecoveryRecordInfo` -- the Raft log index entry -- can hold one by
/// shared pointer off a forward declaration: a nested type cannot be
/// forward-declared, and `mqbs_datastore.h` cannot see `FileStore`.
struct Proposal {
    // INPUT — common
    ...
    // TRAITS
    BSLMF_NESTED_TRAIT_DECLARATION(Proposal, bslma::UsesBslmaAllocator)

    // CREATORS
    explicit Proposal(bslma::Allocator* basicAllocator = 0);
...
class FileStore BSLS_KEYWORD_FINAL : public DataStore {
  ...
  public:
    ...
    /// Rollover requirement of the active file set for a proposal, as
  ...
  private:
    ...
    int openInRecoveryMode(bsl::ostream&          errorDescription,
                           ...
                           bsl::deque<EntryInfo>* recoveryIndex = 0);
  ...
  public:
    ...
    /// `recoveryIndex` with an `EntryInfo` for every journal record after the
    /// rollover boundary, in forward order.  Each carries record metadata
    /// only: what is recovered was written by an earlier incarnation of this
    /// node or by a peer, so no entry comes back owning a write.  Return zero
    /// on success, non-zero value otherwise.
    int openForRaft(bsl::deque<EntryInfo>* recoveryIndex);
    ...
    /// Empty the partition's storages in place, keeping the objects and this
    /// FileStore's references to them.  Invoked on a Raft snapshot install,
    /// before the close: their contents are handles into `d_records`, which
    /// the close clears, but the objects themselves are the ones the queues
    /// were configured with and hold a reference to.  Replacing them instead
    /// would leave every queue on an orphan.  Recovery refills these after the
    /// reopen; `notifyAllQueuesOnRestore` then re-seats the queues.
    void clearStorages();
    ...
    /// ignored.
    void onRecordCommittedPrimary(Proposal&          pw,
    ...
    void undoPropose(const Proposal& pw);
    ...
    int formatMessageRecord(Proposal* pw);
    ...
    int formatQueueCreationRecord(Proposal* pw);
    ...
    int formatSyncPointRecord(Proposal* pw);
    ...
    int formatConfirmRecord(Proposal* pw);
    ...
    int formatDeletionRecord(Proposal* pw);
    ...
    int formatQueuePurgeRecord(Proposal* pw);
    ...
    /// 'pw->d_journalOffset', 'pw->d_entryBlob' (zero-copy mmap alias of the
    /// journal record), and 'pw->d_handle'.
    int formatQueueDeletionRecord(Proposal* pw);
    ...
    /// Re-seat every queue of this partition onto its storage and re-drive
    /// delivery on it, whatever it has taken since the last call.  For a
    /// restore that refills the storages outside the commit path -- an
    /// installed snapshot -- after `clearStorages` emptied them.
    ///
    /// Both halves are needed.  The queues kept their storage objects, but
    /// everything positioned inside them was emptied, so their iterators are
    /// on contents that are gone.  And filling outside the commit path leaves
    /// nothing in the per-queue notification set while opening the receipt
    /// gate on everything brought in, so a PUSH parked in the relay push
    /// stream while the storage was empty has nothing to release it.
    ///
    /// THREAD: This method is called from the partition's dispatcher thread,
    /// which is also the queues' own.
    void notifyAllQueuesOnRestore();
    ...
    /// Return `!isAbsent`: reaching a `FileStore` directly means the legacy
    /// write path, where a record is in the storage from propose, so one the
    /// caller has established to be absent is one that was removed.  A
    /// caller that has only found the message unreceipted says nothing about
    /// the record, and there the answer is to keep waiting: the legacy path
    /// never takes the storage out from under a message that is still
    /// referenced.  On the Raft path the storage's `RecordStore` is the
    /// `PartitionRaft`, which overrides this.
    bool isPendingReplication(mqbi::Storage::DeliveryProbe* probe,
                              bool isAbsent) const BSLS_KEYWORD_OVERRIDE;
    ...
    /// Return `false`: reaching a `FileStore` directly means the legacy
    /// path, which has no leadership transfer.  On the Raft path the
    /// storage's `RecordStore` is the `PartitionRaft`, which overrides this.
    bool isTransferringLeadership() const BSLS_KEYWORD_OVERRIDE;
    ...
    bool
    hasApplied(bsls::Types::Uint64 sequenceNumber) const BSLS_KEYWORD_OVERRIDE;
...
inline bool FileStore::isTransferringLeadership() const
{
    return false;
}
...
inline bool FileStore::isPendingReplication(mqbi::Storage::DeliveryProbe*,
                                            bool isAbsent) const
{
    return !isAbsent;
}
...
inline bool FileStore::hasApplied(
