...
void FileBackedStorage::clear()
{
    // Everything a purge does -- the record handles, the App counters, the
    // capacity meter -- plus the dedup history, which a purge keeps on
    // purpose and this must not.
    purgeQueue();

    // 'purgeQueue' erases message by message, and an erase on this map moves
    // the entry to history rather than removing it: 'find' still returns it,
    // with 'd_isLive' false.  A later DELETION for such a GUID would find
    // that entry and take it for a live message, and 'unlink' asserts on it.
    // The reload is replacing this storage's whole contents, so the history
    // has nothing left to be the history of.
    d_handles.clear();
                                        ...
                                        const DataStoreRecordHandle start)
{
    ...
    if (!d_store_p->isRaft()) {
        // On Raft the handle is kept when the record commits, by
        // 'applyCommittedQueueOp'.  Keeping it here as well would list it
        // twice, and the queue deletion walks this list calling
        // 'removeRecordRaw' on each entry.
                                          ...
                                          const mqbu::StorageKey&   appKey)
{
    ...
    if (!d_store_p->isRaft()) {
        // Same as 'writePurgeRecordImpl': on Raft 'applyCommittedQueueOp'
        // keeps the handle when the record commits.
        d_queueOpRecordHandles.push_back(handle);
    }
