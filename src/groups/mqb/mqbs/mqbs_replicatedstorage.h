...
class ReplicatedStorage : public mqbi::Storage {
  ...
  public:
    ...
    /// Empty this storage of everything tied to the file set behind it, so
    /// that a rebuild can refill it.  Unlike `purge` this keeps nothing: the
    /// dedup history a purge preserves refers to messages that are about to
    /// stop existing, and an entry left there is indistinguishable from a
    /// live one to whatever looks that GUID up next.
    ///
    /// For a Raft snapshot install, which replaces the file set under a queue
    /// that goes on holding this same storage object.
