    ...
    const bsl::shared_ptr<bmqsys::OperationLogger>& opLogger)
{
    ...
    // `opLogger` will return to its object pool on scope exit and it will
    // log the total operation time in that moment.
    ...
    if (event.isControlEvent()) {
        ...
        case MsgChoice::SELECTION_ID_OPEN_QUEUE: {
            ...
            bsl::shared_ptr<bmqsys::OperationLogger> opLogger =
                d_opLoggerPool.getObject();
            opLogger->start()
                << description() << ": open queue [" << queueUri << "]";
        ...
        case MsgChoice::SELECTION_ID_CLOSE_QUEUE: {
            ...
            bsl::shared_ptr<bmqsys::OperationLogger> opLogger =
                d_opLoggerPool.getObject();
            opLogger->start()
                << description() << ": close queue [" << queueUri << "]";
        ...
        case MsgChoice::SELECTION_ID_CONFIGURE_STREAM: {
            const unsigned int qId =
                controlMessage.choice().isConfigureQueueStreamValue()
                    ? controlMessage.choice().configureQueueStream().qId()
                    : controlMessage.choice().configureStream().qId();

            bsl::shared_ptr<bmqsys::OperationLogger> opLogger =
                d_opLoggerPool.getObject();
            opLogger->start()
                << description() << ": configure queue [qId = " << qId << "]";
