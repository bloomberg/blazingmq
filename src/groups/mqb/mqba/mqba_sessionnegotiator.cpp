    ...
    const bsl::string&                description)
{
    ...
    if (negotiationContext->d_connectionType ==
        ...
        // This session should be mapped to the corresponding
        // mqbnet::Cluster, so query ClusterCatalog to look it up: if this
        // is an incoming connection, use the remote peer advertised
        // identity; if this is an outgoing connection, use our own
        // identity (that we embedded in the 'brokerResponse').
    ...
    const InitialConnectionContextSp& context)
{
    ...
    context->negotiationContext()->d_clusterName = userData->d_clusterName;
    if (d_clusterCatalog_p->isMemberOf(
            context->negotiationContext()->d_clusterName)) {
        context->negotiationContext()->d_connectionType =
            mqbnet::ConnectionType::e_CLUSTER_MEMBER;
    }
    else {
        context->negotiationContext()->d_connectionType =
