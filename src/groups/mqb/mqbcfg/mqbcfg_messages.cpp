...
const bdlat_AttributeInfo *TlsConfig::lookupAttributeInfo(
        const char *name,
        int         nameLength)
{
    for (int i = 0; i < 4; ++i) {
...
TlsConfig::TlsConfig(TlsConfig&& original,
                     bslma::Allocator *basicAllocator)
: d_certificateAuthority(bsl::move(original.d_certificateAuthority), basicAllocator)
...
bsl::ostream& TlsConfig::print(bsl::ostream& stream,
                               int           level,
                               int           spacesPerLevel) const
{
    bslim::Printer printer(&stream, level, spacesPerLevel);
    printer.start();
    printer.printAttribute("certificateAuthority", this->certificateAuthority());
