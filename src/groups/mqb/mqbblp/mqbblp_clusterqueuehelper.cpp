...
ClusterQueueHelper::sendReopenQueueRequest(SubQueueContext* subQueueContext,
                                           QueueContext*    queueContext)
{
    ...
    return sendReopenQueueRequest(subQueueContext,
                                  queueContext,
                                  targetNode,
                                  genCount,
                                  1);
                                           ...
                                           int                 numAttempts)
{
    ...
    if (bmqp::QueueUtil::isEmpty(openQueue.handleParameters())) {
        BMQ_LOGTHROTTLE_INFO
            ...
            << "[parameters: " << openQueue.handleParameters()
            << ", reason: 'All read,write,admin counts are <= 0]";
...
//  NORMAL (k_OPEN)
//  ----------------------------------------
//  Close - subtract counters,
//          erase if zeroes,
//          attempt to send
//  Open -  merge counters,
//          attempt to send,
//          cache if send fails,
//          subtract and close if creation fails
//
//  LOST UPSTRAM (state does not change!)
//  ----------------------------------------
//  Close - subtract counters,
//          erase if zeroes,
//          attempt to send
...
//          cache if send fails,
//
//  NEW UPSTRAM (k_CLOSED)
//  ----------------------------------------
//  Do not send Reopen while there are any Open requests in flight
//
//  Close requests  -   subtract counters,
//                      erase if zeroes,
//                      do not attempt send
//  Open requests   -   cache
//
//  NO OPEN REQUESTS IN FLIGHT (k_REOPENING)
//  ----------------------------------------
//  Send Reopen (per subQueue)
//
//  Close requests  -   cache
//  Open requests   -   cache
//
//  REOPEN SUCCEEDS (k_OPEN)
//  REOPEN FAILS (k_CLOSED)
//  ----------------------------------------
//  Send all cached Close (since k_REOPENING)
//
//  All Reopen finish   -   attempt to Send all cached Open (since k_CLOSED)
//
//  This code relies on the fact that there can be Close request before
//  corresponding Open response.
//  There are 3 sets:  Opened, PendingClose, PendingOpen
//  -   Reopen applies to the Opened set
//  -   PendingClose is a subset of Opened.
//  -   Can execute PendingClose before PendingOpen but not before Reopen
//
//  The order of execution when reopening
//  1.  k_CLOSED. Block Open,
//      subtract counters on Close without sending Close request
//  2.  When there are no Open requests in flight, k_REOPENING. Block Close,
//      send Reopen request.
//  3.  On Reopen success.  k_OPEN, unblock Close, unblock Open
//      attempt to Send all cached Close,
//      attempt to Send all cached Open
//
//  4.  On Reopen failure.  k_CLOSED, unblock Close, block Open
//      attempt to Send all cached Close
//
    ...
    mqbnet::ClusterNode*                 responder)
{
    ...
    SubQueueContext& subStream = subStreamIt->value();

    BSLS_ASSERT_SAFE(subStream.d_numOpenRequestsInFlight);

    --subStream.d_numOpenRequestsInFlight;
    ...
    if (mainCode == bmqt::GenericResult::e_SUCCESS) {
        ...
        if (subStream.d_state == SubQueueContext::k_CLOSED) {
            BMQ_LOGTHROTTLE_WARN
                << d_cluster_p->description()
                ...
                << requestContext->response()
                << ", for request: " << requestContext->request();
            if (subStream.d_numOpenRequestsInFlight == 0) {
                sendReopenQueueRequest(&subStream, qcontext);
            }
    ...
    bmqp::QueueUtil::subtractHandleParameters(&subStream.d_parameters,
                                              context->d_handleParameters);
    ...
    if (subStream.d_state == SubQueueContext::k_CLOSED) {
        // Buffer this OpenQueue request until ReopenQueue response
        retry = false;

        if (subStream.d_numOpenRequestsInFlight == 0) {
            BMQ_LOGTHROTTLE_INFO
                << d_cluster_p->description()
                << ": REOPENING upon OpenQueueResponse from "
                << responder->nodeDescription() << ": "
                << requestContext->response()
                << ", for request: " << requestContext->request();
    ...
    if (retry && isQueuePrimaryAvailable(*qcontext, otherThan)) {
    ...
    int                                  numAttempts)
{
    ...
    bdlb::ScopeExitAny completer(
        bdlf::BindUtil::bind(&ClusterQueueHelper::onReopenQueueCompletion,
                             this));
    ...
    bdlb::ScopeExitAny guard(
        bdlf::BindUtil::bind(&ClusterQueueHelper::finishReopening,
                             this,
                             queueContext,
                             sqit));
    ...
    if (bmqt::GenericResult::e_SUCCESS != requestContext->result()) {
        // Can process Close requests instead of parking them
        subQueueContext.d_state = SubQueueContext::k_CLOSED;
    ...
    int                                  numAttempts)
{
    ...
    sendReopenQueueRequest(&subQueueContext,
                           queueContext.get(),
                           activeNode,
                           generationCount,
                           numAttempts + 1);
    ...
    const mqbi::Domain::OpenQueueConfirmationCookie& confirmationCookie)
{
    ...
    d_cluster_p->dispatcher()->execute(
        bdlf::BindUtil::bind(&ClusterQueueHelper::finishOpening,
                             this,
                             context,
                             status,
                             queueHandle,
    ...
    //    if (context->d_clientContext->isClusterMember()) {
    //        d_cluster_p->dispatcher()->execute(
    //            bdlf::BindUtil::bind(&ClusterQueueHelper::respondToOpenQueueRequest,
    //                                 this,
    //                                 context,
    //                                 status,
    //                                 queueHandle,
    ...
    //    }
    //    else {
    //        respondToOpenQueueRequest(context, status,
    //                            queueHandle,
    //                            openQueueResponse,
    //                            confirmationCookie);
    //    }
    ...
    const mqbi::Cluster::HandleReleasedCallback& callback)
{
    ...
    if (state == SubQueueContext::k_REOPENING) {
        ...
        BMQ_LOGTHROTTLE_INFO
            << d_cluster_p->description()
            << ": Parking Close request until Open/Reopen response for"
                                       ...
                                       bsls::Types::Uint64  generationCount)
{
    ...
    for (StreamsMap::iterator iter = queueInfo.d_subQueueIds.begin();
        ...
        if (subQueueContext.d_numOpenRequestsInFlight == 0) {
            rc = sendReopenQueueRequest(&subQueueContext,
                                        queueContext,
                                        activeNode,
                                        generationCount,
                                        1);
        ...
        if (ns) {
            ...
            for (mqbc::ClusterNodeSession::QueueHandleMap::const_iterator cit =
                ...
                cit->second.d_handle_p->deconfigureAll(
                    bdlf::BindUtil::bind(&ClusterQueueHelper::contextHolder,
                                         this,
                                         contextSp,
                                         VoidFunctor()));
            }
