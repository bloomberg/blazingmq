...
void Queue::resetStorage()
{
    ...
    // Synchronous, like 'onReplicatedBatch', and for the same reason: the
    // caller pairs the two, and a dispatched re-seat would land after the
    // nudge it is supposed to precede.
    if (queueEngine()) {
        queueEngine()->resetStorage();
    }
}
