...
struct Event {
    ...
    , d_isHandleKnown(isHandleKnown)
    {
        // PRECONDITIONS
        BSLS_ASSERT_SAFE(d_queueState_p);

        if (d_isHandleKnown) {
            BSLS_ASSERT_SAFE(d_handle_p);
        }
    }
};
    ...
    const mqbi::QueueHandle::HandleConfiguredCallback& configuredCb)
{
    ...
    BSLS_ASSERT_SAFE(isHandleKnown);

    BMQ_LOGTHROTTLE_INFO << "Received success 'configure-stream' response for "
                         << Event(handle, d_queueState_p, isHandleKnown)
                         << ", for parameters " << downStreamParameters;
    ...
    const bsl::shared_ptr<QueueEngineUtil_ReleaseHandleProctor>& proctor)
{
    ...
    if (bmqp_ctrlmsg::StatusCategory::E_SUCCESS == status.category()) {
        ...
        BMQ_LOGTHROTTLE_WARN << "#QUEUE_CLOSE_FAILURE "
    ...
    const bsl::shared_ptr<Routers::AppContext>& routing_sp)
{
    ...
    if (!d_queueState_p->isDeliverConsumerPriority()) {
        if (d_queueState_p->hasMultipleSubStreams()) {
            BMQ_LOGTHROTTLE_ERROR << "#CLIENT_IMPROPER_BEHAVIOR Fanout queue '"
                                  << d_queueState_p->uri()
...
void RelayQueueEngine::resetState(bool isShuttingDown)
{
    ...
    // The engine is starting over, so both iterators start over with it.
...
void RelayQueueEngine::resetStorage()
{
    // The storage was emptied and refilled under this engine.  The two
    // iterators are affected differently.
    //
    // 'd_realStorageIter_mp' held a position among contents that have been
    // freed, and the position it held no longer exists, so it starts over.
    //
    // 'd_storageIter_mp' walks the 'PushStream', which the reload does not
    // touch -- 'FileBackedStorage::purgeQueue' manipulates no QueueEngine --
    // so its position is still a live entry and is kept.  What it holds about
    // that entry is not: the payload aliases a file set that is now closed,
    // the receipt was read before the wipe, and the 'isPendingReplication'
    // probe measured against a commit point the reload has moved.  Keeping
    // that probe would let the entry settle early and be dropped while its
    // record is still on its way.
    d_realStorageIter_mp = storage()->getIterator(
        mqbu::StorageKey::k_NULL_KEY);

    d_storageIter_mp->clearCache();
}
...
int RelayQueueEngine::rebuildInternalState(bsl::ostream& errorDescription)
{
    ...
    for (AppsMap::const_iterator cit = d_apps.cbegin(); cit != d_apps.cend();
        ...
        // No requester to report to -- this rebuild is driven by the reopen,
        // not by a configure request -- so there is no 'ConfigureNotifier'
        // here, only the routing the two calls below need.
        bsl::shared_ptr<Routers::AppContext> routing_sp =
            bsl::allocate_shared<Routers::AppContext>(
                d_allocator_p,
                &d_queueState_p->routingContext());
    ...
    const bmqp::Protocol::SubQueueInfosArray& subQueueIds)
{
    if (d_queueState_p->domain()->cluster()->isRemote()) {
        ...
        if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY(
            ...
            BMQ_LOGTHROTTLE_INFO << d_queueState_p->uri()
                                 << " failed to store GUID [" << msgGUID
                                 << "], result = " << result;
        }
        else {
