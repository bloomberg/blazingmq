...
/// Election timeout, in ticks, that `TestCluster` gives every node.  Named
/// because a test that waits one out has to count the same ticks.
const int k_ELECTION_TIMEOUT_MIN = 10;
  ...
  public:
    ...
    , d_bufferFactory(256, d_allocator_p)
    {
        for (int i = 0; i < numNodes; ++i) {
            ...
            config.d_electionTimeoutMin = k_ELECTION_TIMEOUT_MIN;
...
static void test20_leadershipTransferDeferred()
// LEADERSHIP TRANSFER, TARGET BEHIND
//
// Verify that a transfer to a target that is not caught up arms and waits,
// sending no 'TimeoutNow', and that it fires once the target reports it is
// current.
{
    bmqtst::TestHelper::printTestName("LEADERSHIP TRANSFER, TARGET BEHIND");
    ...
    // Cut the target off, then append an entry, so its 'matchIndex' falls
    // behind the leader's 'lastIndex'.
    cluster.node(leader)->setPeerAvailability(target, false);

    bsl::shared_ptr<bdlbb::Blob> data = cluster.makeBlob("behind");
    RaftNodeOutput               proposeOutput(&alloc);
    BMQTST_ASSERT_EQ(cluster.node(leader)->propose(&proposeOutput, data), 0);
    cluster.node(leader)->flushSends(&proposeOutput);
    cluster.runUntilQuiet(&proposeOutput);

    // The transfer arms rather than sending: no 'TimeoutNow' goes out and
    // the target does not campaign.
    RaftNodeOutput transferOutput(&alloc);
    BMQTST_ASSERT_EQ(cluster.node(leader)->transferLeadership(&transferOutput,
                                                              target),
                     0);

    size_t numTimeoutNow = 0;
    for (size_t i = 0; i < transferOutput.d_messages.size(); ++i) {
        if (transferOutput.d_messages[i].d_type ==
            RaftMessageType::e_TIMEOUT_NOW) {
            ++numTimeoutNow;
        }
    }
    BMQTST_ASSERT_EQ(numTimeoutNow, 0u);
    BMQTST_ASSERT(cluster.node(leader)->isTransferringLeadership());

    cluster.runUntilQuiet(&transferOutput);
    BMQTST_ASSERT_EQ(cluster.node(leader)->state(), RaftState::e_LEADER);

    // Let the target back in.  Its first response carries a 'matchIndex' that
    // reaches 'lastIndex', which is what releases the 'TimeoutNow'.
    cluster.node(leader)->setPeerAvailability(target, true);

    // A flush alone sends nothing: 'nextIndex' already ran ahead, so there
    // are no entries to carry, and the empty message whose response corrects
    // 'matchIndex' only goes out on a heartbeat.  Tick the leader until one
    // is due -- well short of the election timeout that would abandon this.
    for (int i = 0; i < k_ELECTION_TIMEOUT_MIN - 1 &&
                    cluster.node(leader)->isTransferringLeadership();
         ++i) {
        RaftNodeOutput catchUp(&alloc);
        cluster.node(leader)->tick(&catchUp);
        cluster.node(leader)->flushSends(&catchUp);
        cluster.runUntilQuiet(&catchUp);
    }
...
static void test21_leadershipTransferAbandoned()
// LEADERSHIP TRANSFER ABANDONED
//
// Verify that a transfer whose target never catches up is given up after an
// election timeout, leaving this node the leader.  Without this the arming
// is permanent, and 'PartitionRaft::propose' holds every write behind it.
{
    bmqtst::TestHelper::printTestName("LEADERSHIP TRANSFER ABANDONED");

    // Giving up is logged at WARN, and the log stream allocates from the
    // default allocator.  No other case here reaches a severity the test
    // driver emits.
    ...
    // Cut the target off for the whole test: it never reports a 'matchIndex'
    // that reaches 'lastIndex'.
    cluster.node(leader)->setPeerAvailability(target, false);
    ...
    // A second request is refused rather than re-arming: re-arming would
    // restart the clock below, so a caller retrying a transfer that is not
    // converging would keep it alive indefinitely.
    RaftNodeOutput retryOutput(&alloc);
    BMQTST_ASSERT_NE(cluster.node(leader)->transferLeadership(&retryOutput,
                                                              target),
                     0);
    BMQTST_ASSERT_EQ(retryOutput.d_messages.size(), 0u);

    // One election timeout of leader ticks, and no more.  Ticking only the
    // leader keeps the other two from timing out and campaigning, which
    // would clear the arming by stepping this node down instead.
    for (int i = 0; i < k_ELECTION_TIMEOUT_MIN; ++i) {
        BMQTST_ASSERT(cluster.node(leader)->isTransferringLeadership());

        RaftNodeOutput tickOutput(&alloc);
        cluster.node(leader)->tick(&tickOutput);
        cluster.node(leader)->flushSends(&tickOutput);
        cluster.runUntilQuiet(&tickOutput);
    }

    BMQTST_ASSERT(!cluster.node(leader)->isTransferringLeadership());
    BMQTST_ASSERT_EQ(cluster.node(leader)->state(), RaftState::e_LEADER);
    BMQTST_ASSERT_EQ(cluster.node(target)->state(), RaftState::e_FOLLOWER);
}
...
int main(int argc, char* argv[])
{
    ...
    switch (_testCase) {
    case 0:
    case 21: test21_leadershipTransferAbandoned(); break;
    case 20: test20_leadershipTransferDeferred(); break;
