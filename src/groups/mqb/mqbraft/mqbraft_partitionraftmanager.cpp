...
void PartitionRaftManager::stop()
{
    ...
    // 'PartitionRaft::close()', which drops any outstanding proposal
...
void PartitionRaftManager::proposeDeferredSyncPoint(unsigned int partitionId)
{
    // executed by the *CLUSTER DISPATCHER* thread
    ...
    const mqbcmd::StorageCommand& command)
{
    ...
    // loaded once by 'start', so indexing it needs no dispatcher thread -- but
    // it does need this thread's read of them to be ordered after 'start'.
    if (!d_isStarted) {
        result->makeError();
        result->error().message() = "PartitionRaftManager not yet started or "
                                    "is stopping.\n\n";
        return;  // RETURN
    }
                                             ...
                                             const bsl::string&     domainName)
{
    ...
    mqbc::StorageUtil::purgeQueueOnDomain(result, domainName, this, this);
