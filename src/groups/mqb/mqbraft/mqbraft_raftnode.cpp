...
RaftNode::PeerState::PeerState(bslma::Allocator* allocator)
: d_name(allocator)
, d_isAvailable(true)
{
    reset(0);
...
RaftNode::PeerState::PeerState(const bsl::string_view& name,
...
RaftNode::PeerState::PeerState(const PeerState&  other,
                               bslma::Allocator* allocator)
: d_name(other.d_name, allocator)
, d_nextIndex(other.d_nextIndex)
, d_matchIndex(other.d_matchIndex)
, d_snapshotPending(other.d_snapshotPending)
, d_snapshotPendingTicks(other.d_snapshotPendingTicks)
, d_snapshotPendingIndex(other.d_snapshotPendingIndex)
, d_snapshotPendingTerm(other.d_snapshotPendingTerm)
, d_needsSnapshot(other.d_needsSnapshot)
, d_probing(other.d_probing)
, d_probeSent(other.d_probeSent)
, d_stalledTicks(other.d_stalledTicks)
, d_sentCommit(other.d_sentCommit)
, d_isAvailable(other.d_isAvailable)
{
}
...
, d_allocator_p(bslma::Default::allocator(allocator))
{
    ...
    // Note: 'config.d_peers' may be empty -- a single-node cluster has no
    ...
    // Each peer's name is copied into its 'PeerState' here, so 'nodeName'
    // resolves an id through the same map the replication state lives in.
    for (bsl::vector<RaftNodeInfo>::const_iterator it =
             d_config.d_peers.begin();
         it != d_config.d_peers.end();
         ++it) {
        if (it->d_id != d_config.d_selfId) {
            d_peerStates.insert(
                bsl::make_pair(it->d_id, PeerState(it->d_name, allocator)));
        }
    }
...
void RaftNode::breakLeaderCommitContinuity()
{
    ++d_leaderCommitGeneration;
}
                              ...
                              int                 leaderId)
{
    ...
    if (output->d_leaderChanged) {
        // A different leader, or the same one in a different term: what the
        // previous one reported committing says nothing about what this one
        // has, since a leader's commit index starts at its own and climbs.
        breakLeaderCommitContinuity();
    }
    ...
    BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                  << selfName() << ") became FOLLOWER in term "
                  << d_currentTerm << ", leader=" << nodeName(d_leaderId);
}
...
void RaftNode::becomeCandidate(RaftNodeOutput* output, bool preVote)
{
    ...
    if (!preVote) {
        ...
        // Only a real candidacy abandons the leader.  A pre-vote asks whether
        // a term change *would* be supported and leaves the term alone (Raft
        // 9.6), so clearing the leader there would report a leaderless epoch
        // under an unchanged term -- and the term is the partition's leaseId,
        // which the reopen logic reads as one continuous primary epoch.  Peers
        // deny the pre-vote for their own sticky leader anyway, so that would
        // drop a leader for a round this node is about to lose.  A won
        // pre-vote re-enters here with 'preVote == false'.
        d_leaderId              = k_INVALID_NODE_ID;
        output->d_leaderChanged = true;
    }
    else {
    ...
    for (bsl::vector<RaftNodeInfo>::const_iterator it =
        ...
        msg.d_destinationNodeId = it->d_id;
...
void RaftNode::becomeLeader(RaftNodeOutput* output)
{
    ...
    BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                  << selfName() << ") became LEADER in term " << d_currentTerm
                                 ...
                                 const RaftMessage& msg)
{
    ...
    BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                  << selfName() << ") [term " << d_currentTerm << "] "
                  << (grant ? "GRANTS" : "DENIES") << " "
                  << (msg.d_preVote ? "pre-vote" : "vote") << " to "
                  << nodeName(msg.d_sourceNodeId)
                  << " (candidateTerm=" << msg.d_term << ", candidateLastLog=["
                  << msg.d_lastLogIndex << "," << msg.d_lastLogTerm
                  << "], myLastLog=[" << d_log_p->lastIndex() << ","
                  << d_log_p->lastTerm()
                  << "], votedFor=" << nodeName(d_votedFor) << ")";
                                   ...
                                   const RaftMessage& msg)
{
    ...
    if (msg.d_term < d_currentTerm) {
        BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                      << selfName() << ") [term " << d_currentTerm
                      << "] REJECT AppendEntries (stale term) from "
                      << nodeName(msg.d_sourceNodeId)
                      << ", msgTerm=" << msg.d_term;
        RaftMessage resp(d_allocator_p);
    ...
    if (msg.d_prevLogIndex > 0) {
        if (msg.d_prevLogIndex > d_log_p->lastIndex()) {
            BALL_LOG_INFO << "[partition " << d_config.d_partitionId
                          << "] self(" << selfName() << ") [term "
                          << d_currentTerm
                          << "] REJECT AppendEntries (log gap) from "
                          << nodeName(msg.d_sourceNodeId)
                          << ", prevLogIndex=" << msg.d_prevLogIndex
                          << " > myLastIndex=" << d_log_p->lastIndex();

            // The leader answers a rejection by probing backwards, one
            // message at a time, and sends nothing else meanwhile.  What it
            // reported before that is the last word for the whole probe, so
            // it must not be read afterwards as progress since.
            breakLeaderCommitContinuity();
        ...
        if (existingTerm != msg.d_prevLogTerm) {
            BALL_LOG_INFO << "[partition " << d_config.d_partitionId
                          ...
                          << "from " << nodeName(msg.d_sourceNodeId)
                          << ", prevLogIndex=" << msg.d_prevLogIndex
                          << ", myTerm=" << existingTerm
            ...
            // As above: the leader now probes backwards and goes quiet.
            breakLeaderCommitContinuity();
    ...
    // Raft 5.3: what this node has verified against the leader is
    // 'prevLogIndex' plus the entries accepted below, and that -- not the
    // log's own end -- is both what the commit index may advance to and what
    // the response reports.  Entries held above 'prevLogIndex' that this
    // message did not carry were checked against nothing; an empty
    // AppendEntries carries none, so it verifies only 'prevLogIndex'.
    bsls::Types::Uint64 matchIndex = msg.d_prevLogIndex;
    ...
    for (bsl::vector<LogEntry>::size_type i = 0; i < msg.d_entries.size();
        ...
        if (rc != 0) {
            ...
            // 'nextIndex'; the rejection its next message draws is what
            // corrects it.
            BALL_LOG_ERROR << "[partition " << d_config.d_partitionId
                           << "] self(" << selfName()
                           << ") failed to append entry at index "
                           << entryIndex << ", rc=" << rc
                           << "; truncating this batch";
            break;  // BREAK
        }
                                       ...
                                       const RaftMessage& msg)
{
    ...
    // A response carries the term the peer answered in.  A higher one stepped
    // this node down in 'step', so anything left that does not match is from
    // an earlier stint of this node's leadership: its 'matchIndex' describes a
    // log another leader has since truncated, and 'advanceCommitIndex' would
    // count it toward a quorum for entries the peer does not hold.
    if (msg.d_term != d_currentTerm) {
        return;
    }
    ...
    if (msg.d_success) {
        ...
        it->second.d_needsSnapshot         = false;
        ...
        // A peer whose log conflicts rejects every message the leader has in
        // flight, not just the first.  Only the first truncates it; the rest
        // then have a 'prevLogIndex' past its end and report the same last
        // index, so the backoff below computes the same 'nextIndex' for each.
        BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                      << selfName() << ") [term " << d_currentTerm
                      << "] LEADER got REJECT from peer "
                      << nodeName(msg.d_sourceNodeId)
        ...
        if (msg.d_matchIndex < d_log_p->snapshotIndex()) {
            it->second.d_needsSnapshot = true;
        }

        // A peer can report less than it once acked: files wiped, a crash
        // before the acked records reached disk, a snapshot that replaced its
        // log.  Lower 'd_matchIndex' to the report, or 'advanceCommitIndex'
        // counts this peer toward a quorum for entries it no longer holds.
        if (msg.d_matchIndex < it->second.d_matchIndex) {
            it->second.d_matchIndex = msg.d_matchIndex;
        }
                                     ...
                                     const RaftMessage& msg)
{
    ...
    if (msg.d_term < d_currentTerm) {
        BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                      ...
                      << nodeName(msg.d_sourceNodeId) << ", term "
                      << msg.d_term << " < " << d_currentTerm;
        return;
    }

    // Report a term advance even when already a follower with the same
    // leader: observers key their per-term state (elector status, and with it
    // the "CSL backlog applied for this term" signal) off this flag, and a
    // term advance invalidates it.  Mirrors 'becomeFollower'.
    if (msg.d_term > d_currentTerm) {
    ...
    BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                  << selfName() << ") received InstallSnapshot from "
                  << nodeName(msg.d_sourceNodeId)
                                         ...
                                         const RaftMessage& msg)
{
    ...
    if (it->second.d_snapshotPending) {
        BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                      ...
                      << nodeName(msg.d_sourceNodeId) << ", lastIncludedIndex="
    ...
    bsl::vector<RaftMessage>::size_type roundBegin)
{
    ...
    if (nextIdx <= snapshotIndex) {
        ...
        if (nextIdx != snapshotIndex || peer->d_needsSnapshot) {
    ...
    // must be told before it is sent anything past it: a replica appends
    // entries before it applies commits, so entries sent now would land in the
    // file set its rollover is about to replace.  Withhold the entries; the
    // commit rides in the empty message, which the peer caps at
    // 'prevLogIndex' -- at or above 'snapshotIndex' on every path reaching
    // here.  Sending it is enough to release the hold, so the next round
    // resumes entries, and they reach the peer after this message on the same
    // ordered channel.
    ...
    // acked.  Withhold entries once too much is outstanding: a peer that has
    // stopped answering would otherwise be queued the log as fast as it is
    // produced, and the channel buffer neither blocks nor drops.
    ...
    const bool tooManyUnacked = nextIdx > peer->d_matchIndex + 1 +
                                              d_config.d_maxUnackedEntries;

    const bool sendEntries = prevLogIndex < lastIndex && !holdEntries &&
                             !tooManyUnacked;

    if (!sendEntries) {
        // Why an empty message is still worth sending: it carries
        // 'prevLogIndex' and 'leaderCommit', and its response is what corrects
        // 'matchIndex' and 'nextIndex'.  Without it a peer whose 'matchIndex'
        // is still the 'reset' value of 0 -- every peer after 'becomeLeader'
        // or a reconnect -- would have nothing to answer, so nothing would
        // ever correct it.
        if (!holdEntries && !d_heartbeatDue &&
            !(d_config.d_broadcastHeartbeatOnCommit &&
              peer->d_sentCommit < d_commitIndex)) {
            return;  // RETURN
        }
    }
    ...
    if (!msg_p) {
        ...
        // This node's whole commit index, not the end of what this message
        // carries.  Clamping it to the peer's backfill position is a
        // reasonable-looking change and would break
        // 'PartitionRaft::isPendingReplication': that reads the figure as an
        // upper bound on the index of any message the leader has PUSHed, and
        // discards a PUSH whose record it decides can no longer arrive.  A
        // clamped figure can sit below the index of a message already pushed,
        // which would make it discard an acknowledged message.  The receiver
        // clamps for its own commit index ('d_commitIndex') and keeps this
        // one apart, in 'd_lastKnownLeaderCommit'.
...
// PRIVATE ACCESSORS
const char* RaftNode::nodeName(int id) const
{
    if (k_INVALID_NODE_ID == id) {
        // Where a log prints a node that is absent: no leader, no vote cast.
        return "none";  // RETURN
    }

    if (d_config.d_selfId == id) {
        return d_config.d_nodeName.c_str();  // RETURN
    }

    bsl::unordered_map<int, PeerState>::const_iterator it = d_peerStates.find(
        id);

    // Every member of the group has a 'PeerState', carrying its name, from
    // construction on.
    BSLS_ASSERT_SAFE(it != d_peerStates.end());

    return it->second.d_name.c_str();
}

const char* RaftNode::selfName() const
{
    return d_config.d_nodeName.c_str();
}
...
void RaftNode::tick(RaftNodeOutput* output)
{
    ...
    if (d_state == RaftState::e_LEADER) {
        ...
        for (bsl::unordered_map<int, PeerState>::iterator it =
            ...
            if (it->second.d_snapshotPending) {
                if (++it->second.d_snapshotPendingTicks >=
                    ...
                    BALL_LOG_INFO
                        << "[partition " << d_config.d_partitionId << "] self("
                        << selfName()
                        << ") timed out waiting for InstallSnapshotResp "
                        << "from " << it->second.d_name << "; retrying";
                    it->second.d_snapshotPending      = false;
                    it->second.d_snapshotPendingTicks = 0;
                }
            }

            // The response to a probe never arrived, so the guard at the top
            // of 'sendAppendEntries' would suppress this peer for good.  Clear
            // it and let the next round send again.
            //
            // 'nextIndex' is left where it is.  A message at a position the
            // peer does not hold draws a rejection carrying the peer's own
            // position, and that is what corrects it (Raft 5.3: 'nextIndex'
            // decreases on rejection, and on nothing else).  Rewinding here
            // instead put 'nextIndex' at 'matchIndex + 1', which is 1 whenever
            // 'matchIndex' is still the 'reset' value -- below 'snapshotIndex'
            // on a compacted log, so a caught-up peer was sent a full
            // snapshot.
            if (it->second.d_probeSent) {
        ...
        if (k_INVALID_NODE_ID != d_transferTargetId) {
            // The target is still behind.  Under load it may stay behind for
            // as long as this node keeps appending, and the arming is what
            // holds this partition's proposals, so it cannot wait
            // indefinitely (Raft dissertation 3.10: abandon after about an
            // election timeout and resume normal operation).  The caller
            // polls for the new primary and retries.
            if (++d_transferTicks >= d_config.d_electionTimeoutMin) {
                BALL_LOG_WARN
                    << "[partition " << d_config.d_partitionId << "] self("
                    << selfName() << ") abandoning leadership transfer to "
                    << nodeName(d_transferTargetId)
                    << ": it did not catch up within an election timeout.";
                d_transferTargetId = k_INVALID_NODE_ID;
                d_transferTicks    = 0;
            }
        }
...
void RaftNode::step(RaftNodeOutput* output, const RaftMessage& message)
{
    ...
    default: {
        BALL_LOG_WARN << "[partition " << d_config.d_partitionId << "] self("
                      << selfName()
                      ...
                      const bsl::shared_ptr<bdlbb::Blob>& data)
{
    ...
    if (rc != 0) {
        ...
        BALL_LOG_ERROR << "[partition " << d_config.d_partitionId << "] self("
                       << selfName() << ") [term " << d_currentTerm
                                  ...
                                  bsls::Types::Uint64 commitIndex)
{
    ...
    // Not seeded from 'commitIndex': that figure is the caller's, not one a
    // leader reported, and its reader would take it for the latter.  Dropped
    // instead -- the leader sends nothing to a peer while its snapshot is in
    // flight, so whatever was known before is the last word from before a
    // silence and must not read as progress since.
    breakLeaderCommitContinuity();
}
...
void RaftNode::setPeerAvailability(int peerNodeId, bool isAvailable)
{
    ...
    if (peerNodeId == d_leaderId) {
        // The channel to the leader went down or came back.  Either way what
        // it sends next travels a different connection from what it sent
        // before, so arrival order no longer reflects send order and nothing
        // learned earlier can be read as progress since.
        breakLeaderCommitContinuity();
    }

    BALL_LOG_INFO << "[partition " << d_config.d_partitionId << "] self("
                  << selfName() << ") peer " << it->second.d_name << " is now "
...
int RaftNode::transferLeadership(RaftNodeOutput* output, int targetNodeId)
{
    ...
    if (k_INVALID_NODE_ID != d_transferTargetId) {
        // Rejected rather than re-armed.  Re-arming restarts the clock that
        // gives the transfer up, so a caller retrying one that is not
        // converging would keep it alive for as long as it retries -- and
        // with it the hold 'PartitionRaft::propose' puts on this partition's
        // writes.
        BALL_LOG_WARN << "[partition " << d_config.d_partitionId << "] self("
                      << selfName() << ") rejecting leadership transfer to "
                      << nodeName(targetNodeId) << ": a transfer to "
                      << nodeName(d_transferTargetId)
                      << " is already under way.";
        return rc_TRANSFER_IN_PROGRESS;  // RETURN
    }
    ...
    if (ps.d_matchIndex >= d_log_p->lastIndex()) {
        ...
        // once the target reports it is current.  'tick' abandons the
        // transfer if that never happens.
        flushSends(output);
    }
