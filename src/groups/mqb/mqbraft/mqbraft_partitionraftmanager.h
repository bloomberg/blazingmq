...
class PartitionRaftManager : public mqbi::StorageProvider,
  ...
  private:
    ...
    /// `true` between `start` and `stop`.  Atomic because `processCommand`
    /// reads it on the admin thread to order its reads of `d_partitionRafts`
    /// after `start` has loaded them; see
    /// `mqbc::StorageManager::d_isStarted`.
  ...
  public:
    ...
    /// Return the dispatcher client whose thread owns the specified
    /// `partitionId`.  This is the partition's `FileStore`: the
    /// `PartitionRaft` driving it is not a dispatcher client of its own, it
    /// runs on the `FileStore`'s thread.
    mqbi::DispatcherClient* partition(int partitionId) BSLS_KEYWORD_OVERRIDE;

    /// Return the record store driving the specified `partitionId`.
    mqbs::RecordStore* recordStore(int partitionId) BSLS_KEYWORD_OVERRIDE;
    ...
    void proposeDeferredSyncPoint(unsigned int partitionId);
...
inline mqbi::DispatcherClient* PartitionRaftManager::partition(int partitionId)
{
    // executed by *ANY* thread
...
inline mqbs::RecordStore* PartitionRaftManager::recordStore(int partitionId)
{
    // PRECONDITIONS
    BSLS_ASSERT_SAFE(0 <= partitionId &&
                     partitionId < static_cast<int>(d_partitionRafts.size()));

    return d_partitionRafts[partitionId].get();
}
