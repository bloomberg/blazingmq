...
class Queue : public DispatcherClient {
  public:
    ...
    /// Re-seat this queue against its storage, whose contents have been
    /// emptied and are about to be rebuilt.  The storage object itself is
    /// unchanged -- the queue keeps the one it holds -- but anything
    /// positioned inside it is not: an iterator left over the old contents
    /// reads an object that no longer holds what it pointed at, and says
    /// nothing about it.  Used when a partition is rebuilt from an installed
    /// snapshot.
