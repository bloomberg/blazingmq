...
class StorageProvider {
  ...
  public:
    ...
    /// `partitionId`.
    ///
    /// THREAD: Any.  The returned client may only be used to `execute` work
    ///         on, or `synchronize` with, that partition's thread;
    ///         `description` and `inDispatcherThread` are also safe.
    virtual DispatcherClient* partition(int partitionId) = 0;

    /// Return the record store driving the specified `partitionId`: the
    /// `mqbs::FileStore` in legacy/FSM mode, the `mqbraft::PartitionRaft` in
    /// Raft mode.  Nothing on it is safe to touch from another thread -- not
    /// even `isLeader`, which reads state the partition thread writes
    /// unsynchronized -- so reach it from a job sent through `partition`.
    ///
    /// THREAD: `partitionId`s dispatcher thread.
    virtual mqbs::RecordStore* recordStore(int partitionId) = 0;
