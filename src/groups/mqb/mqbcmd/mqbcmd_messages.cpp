        ...
        int                 stringLength)
{
    for (int i = 0; i < 3; ++i) {
        ...
        {
            *result = static_cast<ConsumerStatus::Value>(enumeratorInfo.d_value);
            return 0;
        }
    }
...
const bdlat_AttributeInfo *DomainRemove::lookupAttributeInfo(
        const char *name,
        int         nameLength)
{
    for (int i = 0; i < 2; ++i) {
...
bsl::ostream& DomainRemove::print(bsl::ostream& stream,
                                  int           level,
                                  int           spacesPerLevel) const
{
    bslim::Printer printer(&stream, level, spacesPerLevel);
        ...
        int                 stringLength)
{
    for (int i = 0; i < 3; ++i) {
        ...
        &&  0 == bsl::memcmp(enumeratorInfo.d_name_p, string, stringLength))
        {
            *result = static_cast<EncodingFormat::Value>(enumeratorInfo.d_value);
            return 0;
        }
    }
        ...
        int                 stringLength)
{
    for (int i = 0; i < 3; ++i) {
        ...
        &&  0 == bsl::memcmp(enumeratorInfo.d_name_p, string, stringLength))
        {
            *result = static_cast<FileStoreState::Value>(enumeratorInfo.d_value);
            return 0;
        }
    }
        ...
        int                 stringLength)
{
    for (int i = 0; i < 3; ++i) {
        ...
        &&  0 == bsl::memcmp(enumeratorInfo.d_name_p, string, stringLength))
        {
            *result = static_cast<ResourceUsageMonitorState::Value>(enumeratorInfo.d_value);
            return 0;
        }
    }
    ...
    switch (d_selectionId) {
      ...
      case SELECTION_ID_ROLLOVER: {
        new (d_rollover.buffer())
            Void(original.d_rollover.object());
      } break;
      default:
        BSLS_ASSERT(SELECTION_ID_UNDEFINED == d_selectionId);
    }
...
, d_allocator_p(bslma::Default::allocator(basicAllocator))
{
    switch (d_selectionId) {
      ...
      case SELECTION_ID_REMOVE: {
        new (d_remove.buffer())
            DomainRemove(
                original.d_remove.object(), d_allocator_p);
      } break;
      default:
        BSLS_ASSERT(SELECTION_ID_UNDEFINED == d_selectionId);
    }
...
, d_allocator_p(original.d_allocator_p)
{
    switch (d_selectionId) {
      ...
      case SELECTION_ID_REMOVE: {
        new (d_remove.buffer())
            DomainRemove(
                bsl::move(original.d_remove.object()), d_allocator_p);
      } break;
      default:
        BSLS_ASSERT(SELECTION_ID_UNDEFINED == d_selectionId);
    }
...
DomainRemove& DomainsCommand::makeRemove(const DomainRemove& value)
{
    if (SELECTION_ID_REMOVE == d_selectionId) {
        ...
        reset();
        new (d_remove.buffer())
                DomainRemove(value, d_allocator_p);
        d_selectionId = SELECTION_ID_REMOVE;
    }
...
DomainRemove& DomainsCommand::makeRemove(DomainRemove&& value)
{
    if (SELECTION_ID_REMOVE == d_selectionId) {
        ...
        reset();
        new (d_remove.buffer())
                DomainRemove(bsl::move(value), d_allocator_p);
        d_selectionId = SELECTION_ID_REMOVE;
    }
