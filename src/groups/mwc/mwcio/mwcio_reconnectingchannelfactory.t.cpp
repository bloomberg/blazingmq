...
// mwcio_reconnectingchannelfactory.t.cpp                             -*-C++-*-
...
class Tester : public mwctst::Test {
  private:
    // TYPES
                        ...
                        const ConnectOptions&     options)
{
    PVV("Resolving '" << options.endpoint() << "' to '"
                      << mwcu::PrintUtil::printer(d_resolverResults) << "'");
...
Tester::~Tester()
{
    ...
    d_reconnectingFactory.object()
        .mwcio::ReconnectingChannelFactory ::~ReconnectingChannelFactory();
