...
BALL_LOG_SET_NAMESPACE_CATEGORY("MWCIO.CHANNELUTIL");
...
isValidPacketLength(int*                      packetLength,
                    ...
                    const mwcu::BlobPosition& pos = mwcu::BlobPosition())
{
    void* offsetByte = inBlob.buffer(pos.buffer()).data() + pos.byte();

    if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(
            (mwcu::BlobUtil::bufferSize(inBlob, pos.buffer()) - pos.byte()) >=
                            ...
                            bdlbb::Blob*              inBlob)
{
    ...
    while (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(
        ...
        const int rc =
            mwcu::BlobUtil::findOffset(&offset, *inBlob, offset, packetLength);
...
bool ChannelUtil::isLocalHost(const bsl::string_view& host)
{
    ...
    BSLMT_ONCE_DO
    {
        ...
        bsl::string hostname;
        if (mwcio::ResolveUtil::getHostname(&hostname).code() !=
            ntsa::Error::e_OK) {
        ...
        if (mwcio::ResolveUtil::getIpAddress(&defaultIP, hostname).code() !=
            ntsa::Error::e_OK) {
    ...
    ntsa::Error error = mwcio::ResolveUtil::getIpAddress(&ipAddress, host);
...
bool ChannelUtil::isLocalHost(const ntsa::IpAddress& ip)
{
    ...
    BSLMT_ONCE_DO
    {
        ...
        ntsa::Error error = mwcio::ResolveUtil::getLocalIpAddress(
            &s_localAddresses);
