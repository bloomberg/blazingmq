...
// mwcio_status.h                                                     -*-C++-*-
#ifndef INCLUDED_MWCIO_STATUS
#define INCLUDED_MWCIO_STATUS
...
// mwcio::Status
//
//@DESCRIPTION: This component defines a value semantic type, 'mwcio::Status',
...
class Status {
  private:
    ...
    mutable bdlb::NullableValue<mwct::PropertyBag> d_properties;
    // Optional associated
    // properties
  ...
  public:
    ...
    /// `category() == Status::MWCIO_SUCCESS`) and `false` otherwise.
    operator bool() const;
...
inline mwct::PropertyBag& Status::properties()
{
...
inline const mwct::PropertyBag& Status::properties() const
{
    if (d_properties.isNull()) {
        d_properties.makeValue();
    }
...
inline bsl::ostream& mwcio::operator<<(bsl::ostream&               stream,
                                       mwcio::StatusCategory::Enum value)
{
    return StatusCategory::print(stream, value, 0, -1);
}

inline bsl::ostream& mwcio::operator<<(bsl::ostream&        stream,
                                       const mwcio::Status& value)
{
