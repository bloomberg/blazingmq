...
void executeOnClosedChannelFunc(mwcio::NtcChannel* channel,
                                const Status&      status)
{
    // PRECONDITIONS
    ASSERT(channel);
    ...
    BSLA_MAYBE_UNUSED mwct::PropertyBag& properties = channel->properties();
...
class Tester {
    ...
    mwcio::ChannelFactory::ResultCallback           d_listenResultCallback;
    mwcio::ChannelFactory::ResultCallback           d_connectResultCallback;
    bsl::vector<bsl::shared_ptr<mwcio::Channel> >   d_listenChannels;
    bsl::vector<bsl::shared_ptr<mwcio::Channel> >   d_connectChannels;
  ...
  public:
    ...
    bsl::shared_ptr<mwcio::NtcChannel> connect();
};
...
bsl::shared_ptr<mwcio::NtcChannel> Tester::connect()
{
    bsl::shared_ptr<mwcio::NtcChannel> channel;
    channel.createInplace(d_allocator_p,
                          d_interface_sp,
    ...
    mwcio::ConnectOptions options(d_allocator_p);
    ...
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(status.category(), mwcio::StatusCategory::e_SUCCESS);
...
// ------------------------------------------------------------------------
{
    ...
    bsl::shared_ptr<mwcio::NtcChannel> channel = tester.connect();
    ...
    mwcio::Status status(s_allocator_p);
    channel->write(&status, blob);
    ASSERT_EQ(status.category(), mwcio::StatusCategory::e_SUCCESS);

    mwcio::Channel::CloseFn closeCb = bdlf::BindUtil::bindS(
        s_allocator_p,
