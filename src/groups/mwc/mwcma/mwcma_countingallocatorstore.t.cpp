...
// mwcma_countingallocatorstore.t.cpp                                 -*-C++-*-
...
//
// Plan:
//   1. Supply a 'mwcma::CountingAllocator' ancestor allocator and retrieve
...
// ------------------------------------------------------------------------
{
    ...
    mwcma::CountingAllocator      topAllocator("Top Allocator", s_allocator_p);
    mwcma::CountingAllocatorStore allocatorStore(&topAllocator);
...
//   'mwcma::CountingAllocator' then calls to 'get' will simply return that
//   allocator.
//
// Plan:
//   1. Supply an ancestor allocator that is not a
//      'mwcma::CountingAllocator' and retrieve multiple allocators from
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("ANCESTOR NOT COUNTING ALLOCATOR");

    BSLS_ASSERT_OPT(!dynamic_cast<mwcma::CountingAllocator*>(s_allocator_p) &&
                    "Test allocator is assumed to not be a"
                    " 'mwcma::CountingAllocator'");

    bslma::Allocator*             topAllocator_p = s_allocator_p;
    mwcma::CountingAllocatorStore allocatorStore(topAllocator_p);
