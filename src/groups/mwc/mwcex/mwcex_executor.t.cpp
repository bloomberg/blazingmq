...
/// won't fit in the small on-stack buffer used by `mwcex::Executor` for
/// optimization.
...
class ExecutorElarger {
  ...
  public:
    ...
    {
        mwcex::ExecutorTraits<EXECUTOR>::post(
            d_executor,
    ...
    {
        mwcex::ExecutorTraits<EXECUTOR>::dispatch(
            d_executor,
            BSLS_COMPILERFEATURES_FORWARD(F, f));
    }
...
//
// Plan:
//   1. Default-construct an instance of 'mwcex::Executor'. Check
//      postconditions.
//
//   2. Construct an instance of 'mwcex::Executor' from another executor.
//      Check postconditions.
//
//   3. Copy-construct an instance of 'mwcex::Executor'. Check
//      postconditions.
//
//   4. Move-construct an instance of 'mwcex::Executor'. Check
//      postconditions.
//
// Testing:
//   - default constructor
...
{
    // PRECONDITIONS
    BSLMF_ASSERT(mwcex::Executor_Box_SboImpCanHold<StatExecutor>::value ==
                 true);
    BSLMF_ASSERT(mwcex::Executor_Box_SboImpCanHold<
    ...
    // 2. construct from another executor
    {
        ...
        // create an 'mwcex::Executor' from a small executor to enable SBO
        mwcex::Executor ex1(smallEx, &alloc);

        // check postconditions
        ASSERT(ex1.target<StatExecutor>());
        ASSERT(*ex1.target<StatExecutor>() == smallEx);

        // create an 'mwcex::Executor' from a large executor to disable SBO
        mwcex::Executor ex2(largeEx, &alloc);
    ...
    // 3. copy constructor
    {
        ...
        mwcex::Executor ex1;                   // empty
        mwcex::Executor ex2(smallEx, &alloc);  // contains small target
        mwcex::Executor ex3(largeEx, &alloc);  // contains large target

        // copy an empty executor
        mwcex::Executor ex1Copy(ex1);

        // check postconditions
        ASSERT(!ex1Copy);

        // copy an empty executor specifying an (to be ignored) allocator
        mwcex::Executor ex1CopyAlloc(ex1, &alloc);
    ...
    // 4. move constructor
    {
        ...
        mwcex::Executor ex1Copy(bslmf::MovableRefUtil::move(ex1));

        // check postconditions
        ASSERT(!ex1Copy);

        // move an empty executor specifying an (to be ignored) allocator
        mwcex::Executor ex1CopyAlloc(bslmf::MovableRefUtil::move(ex1), &alloc);
        ...
        mwcex::Executor ex2Copy(bslmf::MovableRefUtil::move(ex2));
        ...
        mwcex::Executor ex3Copy(bslmf::MovableRefUtil::move(ex3));
...
//
// Plan:
//   1. Executor-assign an instance of 'mwcex::Executor'. Check
//      postconditions.
//
//   2. Copy-assign an instance of 'mwcex::Executor'. Check
//      postconditions.
//
//   3. Move-assign an instance of 'mwcex::Executor'. Check
//      postconditions.
//
//   4. Assign a new target to an instance of 'mwcex::Executor' via the
...
//
// Plan:
//   Create an instance of 'mwcex::Executor' 'ex1' by constructing
...
// ------------------------------------------------------------------------
{
    bslma::TestAllocator     alloc;
    StatExecutor::Statistics estat;
    mwcex::Executor          ex(StatExecutor(&estat), &alloc);
...
// ------------------------------------------------------------------------
{
    ...
    // 1. small targets
    {
        ...
        mwcex::Executor ex1(smallEx1, &alloc);
        mwcex::Executor ex2(smallEx2, &alloc);
    ...
    // 2. large targets
    {
        ...
        mwcex::Executor ex1(largeEx1, &alloc);
        mwcex::Executor ex2(largeEx2, &alloc);
...
// ------------------------------------------------------------------------
{
    ...
    StatExecutor::Statistics estat;
    mwcex::Executor          ex1;
    mwcex::Executor          ex2(StatExecutor(&estat), &alloc);
...
// ------------------------------------------------------------------------
{
    ...
    ASSERT_EQ(mwcex::Executor() == mwcex::Executor(), true);
    ASSERT_EQ(mwcex::Executor() != mwcex::Executor(), false);

    // empty executor is not equal to a non-empty executor
    ASSERT_EQ(mwcex::Executor() ==
                  mwcex::Executor(SelfEqualExecutor(), &alloc),
              false);
    ASSERT_EQ(mwcex::Executor() !=
                  mwcex::Executor(SelfEqualExecutor(), &alloc),
              true);
    ...
    ASSERT_EQ(mwcex::Executor(SelfEqualExecutor(), &alloc) ==
                  mwcex::Executor(),
              false);
    ASSERT_EQ(mwcex::Executor(SelfEqualExecutor(), &alloc) !=
                  mwcex::Executor(),
    ...
    mwcex::Executor ex1(ExecutorElarger<SelfUnequalExecutor>(), &alloc);
    mwcex::Executor ex2 = ex1;
    ...
    ASSERT_EQ(mwcex::Executor(SelfEqualExecutor(), &alloc) ==
                  mwcex::Executor(SelfUnequalExecutor(), &alloc),
              false);
    ASSERT_EQ(mwcex::Executor(SelfEqualExecutor(), &alloc) !=
                  mwcex::Executor(SelfUnequalExecutor(), &alloc),
              true);
    ...
    // compare equal ...
    ASSERT_EQ(mwcex::Executor(SelfUnequalExecutor(), &alloc) ==
                  mwcex::Executor(SelfUnequalExecutor(), &alloc),
              false);
    ASSERT_EQ(mwcex::Executor(SelfUnequalExecutor(), &alloc) !=
                  mwcex::Executor(SelfUnequalExecutor(), &alloc),
