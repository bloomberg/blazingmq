...
// mwcex_strand.h                                                     -*-C++-*-
#ifndef INCLUDED_MWCEX_STRAND
#define INCLUDED_MWCEX_STRAND

//@PURPOSE: Provides a strand execution context.
//
//@CLASSES:
//  mwcex::Strand:         a strand execution context
//  mwcex::StrandExecutor: an executor to submit work to the strand
//
//@DESCRIPTION:
// This component provides a mechanism, 'mwcex::Strand', that is an execution
// context (see package documentation) that provides guarantees of ordering and
// non-concurrency.
//
/// Thread safety
///-------------
// 'mwcex::Strand' is fully thread-safe, meaning that multiple threads may use
...
// function, 'mwcex::StrandExecutor' is fully thread-safe, meaning that
...
//..
//  // create a strand
//  mwcex::Strand<EX> strand(ex);
//
//  // obtain an executor
//  mwcex::Strand<EX>::ExecutorType strandEx = strand.executor();
//
//  // start the strand
//  strand.start();
//
...
template <class EXECUTOR>
inline bool
mwcex::operator==(const StrandExecutor<EXECUTOR>& lhs,
                  const StrandExecutor<EXECUTOR>& rhs) BSLS_KEYWORD_NOEXCEPT
{
    return &lhs.context() == &rhs.context();
}
...
template <class EXECUTOR>
inline void mwcex::swap(StrandExecutor<EXECUTOR>& lhs,
                        StrandExecutor<EXECUTOR>& rhs) BSLS_KEYWORD_NOEXCEPT
{
    lhs.swap(rhs);
}
