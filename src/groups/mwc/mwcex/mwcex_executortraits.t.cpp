...
//   'mwcex::ExecutorTraits<E>::post(e, f)' results in a call to
//   'e.post(f)'.
//
// Testing:
//   mwcex::ExecutorTraits::post
// ------------------------------------------------------------------------
{
    ...
    bool executed = false;
    mwcex::ExecutorTraits<PostDispatchExecutor>::post(
        executor,
...
//      operations. Check that 'mwcex::ExecutorTraits<E>::dispatch(e, f)'
...
// ------------------------------------------------------------------------
{
    // 1. dispatchable executor
    {
        ...
        bool executed = false;
        mwcex::ExecutorTraits<PostDispatchExecutor>::dispatch(
            executor,
            SetFlagOnCall(&executed));
        ASSERT(executed);
    ...
    // 2. non-dispatchable executor
    {
        ...
        bool executed = false;
        mwcex::ExecutorTraits<PostExecutor>::dispatch(
            executor,
            SetFlagOnCall(&executed));
        ASSERT(executed);
        ASSERT_EQ(executor.d_postCallCount, 1);
