...
// mwcex_sequentialcontext.h                                          -*-C++-*-
#ifndef INCLUDED_MWCEX_SEQUENTIALCONTEXT
#define INCLUDED_MWCEX_SEQUENTIALCONTEXT

//@PURPOSE: Provides a single-threaded execution context.
//
//@CLASSES:
//  mwcex::SequentialContext:         a single-threaded execution context
//  mwcex::SequentialContextExecutor: an executor to submit work to the context
//
//@SEE ALSO:
//  mwcex_strand
//
...
//
/// Comparison to mwcex::Strand
///---------------------------
// While 'mwcex::Strand' is an execution context adapter created on top of an
// existing execution context, 'mwcex::SequentianContext' does not require an
// underlying context to work. Otherwise, they are very similar in their
// purpose and functions.
//
/// Thread safety
///-------------
// 'mwcex::SequentialContext' is fully thread-safe, meaning that multiple
...
// function, 'mwcex::SequentialContextExecutor' is fully thread-safe, meaning
...
//..
//  mwcex::SequentialContext context;
//  int rc = context.start();
//  BSLS_ASSERT(rc == 0);
//..
// Obtain an executor object:
//..
//  mwcex::SequentialContext::ExecutorType executor = context.executor();
//..
...
class SequentialContext {
  ...
  public:
    ...
    /// `mwcsys::ThreadUtil::defaultAttributes()`. Return 0 on success, and
...
// FREE OPERATORS
inline bool
mwcex::operator==(const SequentialContextExecutor& lhs,
                  const SequentialContextExecutor& rhs) BSLS_KEYWORD_NOEXCEPT
{
    return &lhs.context() == &rhs.context();
}
...
inline void mwcex::swap(SequentialContextExecutor& lhs,
                        SequentialContextExecutor& rhs) BSLS_KEYWORD_NOEXCEPT
{
    lhs.swap(rhs);
}
