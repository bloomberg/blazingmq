...
//
// Plan:
//   1. Default-construct a 'mwcex::Promise'. Check that it has a shared
//      state.
//
//   2. Clock-construct a 'mwcex::Promise'. Check that it has a shared
//      state.
//
//   3. Move-construct a 'mwcex::Promise'. Check that the moved-to object
//      refers to the shared state of the moved-from object.
//
//   4. Destroy a promise object without making the shared state ready.
//      Check that the shared state contains an 'mwcex::PromiseBroken'
//      exception.
//
// Testing:
//   mwcex::Promise's default constructor
//   mwcex::Promise's clock constructor
//   mwcex::Promise's move constructor
//   mwcex::Promise's destructor
// ------------------------------------------------------------------------
{
    ...
    {
        mwcex::Promise<int> promise(&alloc);
    ...
    // 2. clock constructor
    {
        mwcex::Promise<int> promise(bsls::SystemClockType::e_REALTIME, &alloc);
    ...
    // 3. move constructor
    {
        ...
        mwcex::Promise<int> movedFromPromise(&alloc);
        movedFromPromise.setValue(42);

        // create a moved-to promise
        mwcex::Promise<int> movedToPromise(
    ...
    // 4. destructor
    {
        ...
        // the shared state contains a 'mwcex::PromiseBroken' exception
        bool exceptionThrown = false;
        try {
            future.get();
        }
        catch (const mwcex::PromiseBroken&) {
...
//
// Plan:
//   Move-assign a 'mwcex::Promise'. Check that the moved-to object refers
//   to the shared state of the moved-from object.
//
// Testing:
//   mwcex::Promise's move assignment operator
// ------------------------------------------------------------------------
{
    bslma::TestAllocator alloc;
    ...
    // create a moved-to promise
    mwcex::Promise<int> movedToPromise(&alloc);
...
// ------------------------------------------------------------------------
{
    ...
    // create promises
    mwcex::Promise<int> promise1(&alloc);
    mwcex::Promise<int> promise2(&alloc);
...
//
// Testing:
//   mwcex::PromiseBroken::what
// ------------------------------------------------------------------------
{
    mwcex::PromiseBroken promiseBroken;
