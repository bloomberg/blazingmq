...
/// type `mwcex::FutureResult<R1>` and returns a value of type `R2`.
...
struct DummyContinuationFunction {
    ...
    /// Not implemented.
    R2 operator()(mwcex::FutureResult<R1>) const;
};
...
struct SetValueContinuation {
    ...
    // ACCESSORS
    void operator()(mwcex::FutureResult<VALUE> result) const
    {
...
struct ForwardValueContinuation {
    ...
    // ACCESSORS
    VALUE operator()(mwcex::FutureResult<VALUE> result) const
    {
        return result.get();
    }
};
...
//
// Plan:
//   Check that the 'mwcex::ExecutionUtil::ExecuteResult' metafunction
...
//:
//:  o If 'P' is a Two-Way policy - 'mwcex::Future<R>'.
//
// Testing:
//   mwcex::ExecutionUtil::ExecuteResult
// ------------------------------------------------------------------------
{
    // One-Way policy
    {
        ...
        ASSERT((bsl::is_same<mwcex::ExecutionUtil::ExecuteResult<
                                 PolicyType,
    ...
    // Two-Way policy
    {
        ...
        ASSERT((bsl::is_same<mwcex::ExecutionUtil::ExecuteResult<
                                 PolicyType,
                                 DummyNullaryFunction<void> >::Type,
                             mwcex::Future<void> >::value));

        ASSERT((bsl::is_same<mwcex::ExecutionUtil::ExecuteResult<
                                 PolicyType,
                                 DummyNullaryFunction<int> >::Type,
                             mwcex::Future<int> >::value));
    ...
    {
        typedef mwcex::ExecutionPolicy<
            mwcex::ExecutionProperty::TwoWayR<float> >
            PolicyType;

        ASSERT((bsl::is_same<mwcex::ExecutionUtil::ExecuteResult<
                                 PolicyType,
                                 DummyNullaryFunction<void> >::Type,
                             mwcex::Future<float> >::value));
...
//
// Plan:
//   Check that the 'mwcex::ExecutionUtil::ThenExecuteResult' metafunction
//   conforms to the specification. That is:
//
//   Given:
//:  o A type 'P', that is an execution policy type (see
//:   'mwcex_executionpolicy').
//:
//:  o A type 'FT', that is 'mwcex::Future<R1>'.
//:
...
//:    otherwise.
//
//   The type of 'mwcex::ExecutionUtil::ThenExecuteResult<P, FT, FN>::Type'
//   expression is the follow:
//:  o If 'P' is a One-Way policy - 'void'.
//:
//:  o If 'P' is a Two-Way policy - 'mwcex::Future<R2>'.
//
// Testing:
//   mwcex::ExecutionUtil::ThenExecuteResult
// ------------------------------------------------------------------------
{
    // One-Way policy
    {
        ...
        ASSERT((bsl::is_same<mwcex::ExecutionUtil::ThenExecuteResult<
                                 PolicyType,
                                 mwcex::Future<void>,
...
// ------------------------------------------------------------------------
{
    ...
    {
        // submit function object for execution
        mwcex::ExecutionUtil::execute(mwcex::ExecutionPolicyUtil::oneWay()
                                          .neverBlocking()
                                          .useExecutor(context.executor())
                                          .useAllocator(&alloc),
                                      ThrowOnCall());
...
//:       completion of the submitted function object invocation;
//:     o The function object is executed as if by a call to 'ex.post(f)';
//:     o The execution function returns a 'mwcex::Future' object that
...
//      Check that the execution function returns a 'mwcex::Future' object
...
// ------------------------------------------------------------------------
{
    ...
    // 1. General use-case.
    {
        ...
        // submit function object for execution
        mwcex::Future<void> result = mwcex::ExecutionUtil::execute(
    ...
    {
        // submit function object for execution
        mwcex::Future<ReturnValueOnCall::ResultType> result =
    ...
    {
        // submit function object for execution
        mwcex::Future<ReturnReferenceOnCall::ResultType> result =
            mwcex::ExecutionUtil::execute(mwcex::ExecutionPolicyUtil::twoWay()
                                              .neverBlocking()
                                              .useExecutor(context.executor())
                                              .useAllocator(&alloc),
    ...
    {
        // submit function object for execution
        mwcex::Future<ReturnVoidOnCall::ResultType> result =
            mwcex::ExecutionUtil::execute(mwcex::ExecutionPolicyUtil::twoWay()
                                              .neverBlocking()
                                              .useExecutor(context.executor())
                                              .useAllocator(&alloc),
    ...
    // 5. Submitted function object throws an exception.
    {
        ...
        // submit function object for execution
        mwcex::Future<ThrowOnCall::ResultType> result =
            mwcex::ExecutionUtil::execute(
                mwcex::ExecutionPolicyUtil::twoWay()
                    .neverBlocking()
...
//      Blocking execution policy, and such that 'f()' throws an exception.
//      Check that the execution function return a 'mwcex::Future' object
//      to be containing the thrown exception.
//
// Testing:
//   mwcex::ExecutionUtil::execute
...
//      Blocking execution policy, and such that 'f()' returns a value.
//      Check that the execution function returns a ready 'mwcex::Future'
...
//
// Testing:
//   mwcex::ExecutionUtil::thenExecute
// ------------------------------------------------------------------------
{
    bslma::TestAllocator alloc;
    TestExecutionContext context(&alloc);
    ...
    {
        // prepare a future
        bsl::shared_ptr<mwcex::FutureSharedState<int> > sharedState =
            bsl::allocate_shared<mwcex::FutureSharedState<int> >(&alloc);
        ...
        mwcex::ExecutionUtil::thenExecute(mwcex::ExecutionPolicyUtil::oneWay()
                                              .alwaysBlocking()
                                              .useExecutor(context.executor())
                                              .useAllocator(&alloc),
                                          future,
        ...
        // 'mwcex::ExecutionUtil::execute' with the same policy as passed to
        // 'thenExecute'
        ASSERT_EQ(context.statistics().d_dispatchCount, 1);
    }
...
// ------------------------------------------------------------------------
{
    ...
    // 2. shared state lifetime
    {
        ...
        // attach a continuation
        mwcex::ExecutionUtil::thenExecute(mwcex::ExecutionPolicyUtil::twoWay()
                                              .neverBlocking()
                                              .useExecutor(context.executor())
                                              .useAllocator(&alloc),
                                          mwcex::Future<int>(sharedState),
...
// ------------------------------------------------------------------------
{
    ...
    // 1. General use-case
    {
        ...
        // submit function object for execution
        mwcex::ExecutionUtil::invoke(context.executor(), toBeExecuted);
    ...
    {
        // submit function object for execution
        ReturnValueOnCall::ResultType result = mwcex::ExecutionUtil::invoke(
            context.executor(),
            ReturnValueOnCall());
    ...
    // 3. Submitted function object returns a reference.
    {
        ...
        ReturnReferenceOnCall::ResultType result =
            mwcex::ExecutionUtil::invoke(context.executor(),
                                         ReturnReferenceOnCall());
    ...
    {
        // submit function object for execution
        mwcex::ExecutionUtil::invoke(context.executor(), ReturnVoidOnCall());
    }
    ...
    // 5. Submitted function object throws an exception.
    {
        ...
        bool exceptionThrown = false;
        try {
            mwcex::ExecutionUtil::invoke(context.executor(), ThrowOnCall());
        }
        catch (const ThrowOnCall::ExceptionType&) {
            exceptionThrown = true;
        }
