...
//    everything
{
...

    {
        ...
        mwcu::BlobObjectProxy<TestHeader> pxy(&blob);
        ASSERT(pxy.isInBlob());
...

    {
        ...
        mwcu::BlobObjectProxy<TestHeader> pxy(&blob, 9);
        ASSERT(!pxy.isInBlob());
        ASSERT_EQ(pxy->d_member1, 1u);
...

    {
        ...
        mwcu::BlobObjectProxy<TestHeader> pxy(&blob, -1);
        ASSERT(pxy.isInBlob());
...

    {
        ...
        mwcu::BlobObjectProxy<TestHeader> pxy(&blob, 8);
        ASSERT(!pxy.isInBlob());
        ASSERT(pxy.hasMember(&TestHeader::d_member1));
        ASSERT(!pxy.hasMember(&TestHeader::d_member2));
...

    {
        ...
        mwcu::BlobObjectProxy<TestHeader> pxy;
        pxy.reset(&blob,
                  mwcu::BlobPosition(),
...

    {
        ...
        ASSERT_EQ(pos, mwcu::BlobPosition(0, sizeof(TestHeader)));

        // Looking at part of an object
        pxy.reset(&blob, 10);
        pxy.loadEndPosition(&pos);
        ASSERT_EQ(pos, mwcu::BlobPosition(0, 10));

        // Looking at an object at the end of the blob
        pxy.reset(&blob, mwcu::BlobPosition(0, sizeof(TestHeader)));
        pxy.loadEndPosition(&pos);
        ASSERT_EQ(pos, mwcu::BlobPosition(1, 0));
    }
}
...
//
{
    mwctst::TestHelper::printTestName("Usage Example 1 Test");
...
// --------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Usage Example 2 Test");
