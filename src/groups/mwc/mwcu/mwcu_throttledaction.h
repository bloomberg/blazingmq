...
// mwcu_throttledaction.h                                             -*-C++-*-
#ifndef INCLUDED_MWCU_THROTTLEDACTION
#define INCLUDED_MWCU_THROTTLEDACTION

//@PURPOSE: Provide macros and utilities to throttle an action.
//
//@CLASSES:
//  mwcu::ThrottledActionParams: struct holding the state of a throttleable
//                               action.
//
//@MACROS:
//  MWCU_THROTTLEDACTION_THROTTLE(P, ACTION):                   Reference below
//  MWCU_THROTTLEDACTION_THROTTLE_WITH_RESET(P, ACTION, RESET): Reference below
//  MWCU_THROTTLEDACTION_THROTTLE_NO_RESET(P, ACTION):          Reference below
//
...
//
//  MWCU_THROTTLEDACTION_THROTTLE_WITH_RESET(P, ACTION, RESET)
//      // Execute the specified 'ACTION' having the specified throttling
...
//..
//  mwcu::ThrottledActionParams myThrottledLog(5000, 3); // No more than 3 logs
//                                                       //  in a 5s timeframe
//  // Note that the ThrottledActionParams should be a class member, or a
//  // static variable.
//
//  MWCU_THROTTLEDACTION_THROTTLE(myThrottledLog,
...
struct ThrottledActionParams {
  public:
    ...
    /// Helper constructor to create a `mwcu::ThrottledActionParams` object
...
#define MWCU_THROTTLEDACTION_THROTTLE(P, ACTION)                              \
    MWCU_THROTTLEDACTION_THROTTLE_INTERNAL(P, ACTION, {                       \
...
#define MWCU_THROTTLEDACTION_THROTTLE_WITH_RESET(P, ACTION, RESET)            \
    MWCU_THROTTLEDACTION_THROTTLE_INTERNAL(P, ACTION, RESET)
// Throttle the specified 'ACTION' using the specified parameters 'P' and
...
#define MWCU_THROTTLEDACTION_THROTTLE_NO_RESET(P, ACTION)                     \
    MWCU_THROTTLEDACTION_THROTTLE_INTERNAL(P, ACTION, {})
// Throttle the specified 'ACTION' using the specified parameters 'P' and
// use a void reset action.

#define MWCU_THROTTLEDACTION_THROTTLE_INTERNAL(P, ACTION, RESET)              \
    {                                                                         \
