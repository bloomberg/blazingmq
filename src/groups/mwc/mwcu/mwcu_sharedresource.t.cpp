...
void MyService::asyncDoStuff()
{
    ...
    d_threadPool_p->enqueueJob(
        mwcu::WeakMemFnUtil::weakMemFn(&MyService::doStuff,
                                       d_self.acquireWeak()));
}
...
//
// Plan:
//   1. Create a 'mwcu::SharedResource' managing no resource by default-
//      constructing a 'mwcu::SharedResource'. Check that no memory was
//      allocated and that the resource is invalid.
//
//   2. Create a 'mwcu::SharedResource' managing no resource by nullptr-
//      constructing a 'mwcu::SharedResource'. Check that no memory was
//      allocated and that the resource is invalid.
//
//   3. Create a 'mwcu::SharedResource' managing a resource. Check that
...
//      memory was released.
//
//   4. Try create a 'mwcu::SharedResource' object specifying a resource, a
//      deleter and an allocator that will throw on use. Check that the
//      deleter is used to free the resource.
//
// Testing:
//   mwcu::SharedResource's default constructor
//   mwcu::SharedResource's nullptr constructor
//   mwcu::SharedResource's resource constructor
//   mwcu::SharedResource's destructor
// ------------------------------------------------------------------------
{
    ...
    // 1. empty resource (default-constructed)
    {
...

        {
            ...
            mwcu::SharedResource<int> sharedResource(&alloc);
    ...
    // 2. empty resource (nullptr-constructed)
    {
        ...
        {
            // create shared resource
            mwcu::SharedResource<int> sharedResource(bsl::nullptr_t(), &alloc);
    ...
    // 3. non-empty resource
    {
...

        {
            ...
            // create shared resource
            mwcu::SharedResource<int> sharedResource(&resource, &alloc);
    ...
    // 4. exception safety
    {
        ...
        // try to create a 'mwcu::SharedResource', which should fail
        bool exceptionThrown = false;
        try {
            mwcu::SharedResource<int, Deleter> sharedResource(
                resource,
...
//
// Testing:
//   mwcu::SharedResource::acquire
//   mwcu::SharedResource::acquireWeak
// ------------------------------------------------------------------------
{
    bslma::TestAllocator alloc;
    int                  resource = 42;
    ...
    {
        mwcex::SequentialContext context(&alloc);
        ...
        context.executor().post(
            bdlf::MemFnUtil::memFn(&mwcu::SharedResource<int>::invalidate,
                                   &sharedResource));
...
//
// Testing:
//   mwcu::SharedResource::invalidate
// ------------------------------------------------------------------------
{
    bslma::TestAllocator alloc;
    ...
    {
        typedef mwcu::SharedResourceFactoryDeleter<Sleep, bslma::Allocator>
            Deleter;
        ...
        // create shared resource
        mwcu::SharedResource<Sleep, Deleter> sharedResource(
            sleep,
            Deleter(&resourceFactory),
            &alloc);
...
//      resource now manages the new target.
//
//   5. Create a 'mwcu::SharedResource' object specifying a resource, a
...
//      the 'mwcu::SharedResource' object is left in managing no resource.
//
// Testing:
//   mwcu::SharedResource::reset
// ------------------------------------------------------------------------
{
    bslma::TestAllocator alloc;
    ...
    // 2. reset non-empty to empty
    {
        ...
        // create shared resource
        mwcu::SharedResource<int> sharedResource(&resource1, &alloc);
    ...
    // 5. exception safety
    {
        ...
        // the 'mwcu::SharedResource' is left invalid
        ASSERT(!sharedResource.isValid());
    }
}
...
//
// Concerns:
//   Ensure proper behavior of the 'mwcu::SharedResourceFactoryDeleter'
//   class.
//
// Plan:
...
//
// Testing:
//   mwcu::SharedResourceFactoryDeleter
// ------------------------------------------------------------------------
{
    ...
    mwcu::SharedResourceFactoryDeleter<int, bslma::Allocator> deleter(
        &factory);
    deleter(resource);
