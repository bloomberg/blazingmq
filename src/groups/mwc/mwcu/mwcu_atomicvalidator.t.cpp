...
void acquireReleaseThread(mwcu::AtomicValidator* validator,
...
// ------------------------------------------------------------------------
{
...

    {
        ...
        PVV("- VALIDATOR NULL");
        {
            mwcu::AtomicValidatorGuard obj0(0);
            ASSERT(obj0.isValid());
            ASSERT(obj0.release() == 0);
            ASSERT(obj0.isValid());
        }

        mwcu::AtomicValidator validator;

        PVV("- VALIDATOR VALID");
        {
            mwcu::AtomicValidatorGuard obj(&validator);  // ACQUIRE #1

            ASSERT(obj.isValid());

            mwcu::AtomicValidator* releaseRC = obj.release();
        ...
        PVV("- VALIDATOR INVALID");
        {
            ...
            mwcu::AtomicValidatorGuard obj(&validator);
        ...
        PVV("- VALIDATOR VALID, PRE-ACQUIRED");
        {
            ...
            mwcu::AtomicValidatorGuard obj(&validator, 1);  // no acquisition
            ASSERT(obj.isValid());
        ...
        {
            mwcu::AtomicValidatorGuard obj(&validator, 0);  // ACQUIRE #3
            ASSERT(obj.isValid());
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("ATOMIC VALIDATOR - MULTI-THREADED");
