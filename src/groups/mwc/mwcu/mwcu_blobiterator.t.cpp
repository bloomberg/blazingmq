...
// ------------------------------------------------------------------------
{
    ...
    {
        PVV("Default ctor");
        mwcu::BlobIterator obj;

        PVVV("iterator = " << obj);

        ASSERT_EQ(true, obj.atEnd());
        ASSERT_EQ(0, obj.remaining());
        ASSERT_EQ(mwcu::BlobPosition(), obj.position());
        ASSERT(!obj.blob());
    }

    {
        ...
        mwcu::BlobIterator obj(&blob, mwcu::BlobPosition(12, 0), 4, true);

        PVVV("iterator = " << obj);

        ASSERT_EQ(false, obj.atEnd());
        ASSERT_EQ(4, obj.remaining());
        ASSERT_EQ(mwcu::BlobPosition(12, 0), obj.position());
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Advance Test");
    ...
    for (int dataIdx = 0; dataIdx < NUM_DATA; ++dataIdx) {
        ...
        mwcu::BlobPosition startPos(data.d_startBuffer, data.d_startByte);
        mwcu::BlobIterator iter(&blob,
                                startPos,
        ...
        mwcu::BlobPosition expectedPos(data.d_expectedBuffer,
                                       data.d_expectedByte);
...
// --------------------------------------------------------------------
{
    ...
    for (int dataIdx = 0; dataIdx < NUM_DATA; ++dataIdx) {
        ...
        blob.setLength(BLOB_LENGTH);
        mwcu::BlobIterator iter(&blob,
                                mwcu::BlobPosition(data.d_startBuffer,
                                                   data.d_startByte),
                                data.d_length,
                                true);
        mwcu::BlobIterator iter2(iter), iter3(iter), iter4, end;
        iter4 = iter;
        ASSERT_EQ_D("line " << data.d_line, iter, iter4);

        for (int i = 0; i < data.d_length; ++i) {
            ASSERT_NE_D("line " << data.d_line, iter, end);
            const mwcu::BlobPosition p(iter.position());
            char                     c;
            const int rc = mwcu::BlobUtil::readNBytes(&c, blob, p, 1);
