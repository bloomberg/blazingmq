...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Position Breathing Test");

    {
        PVV("Default constructor");
        mwcu::BlobPosition obj;
    ...
    // Verifying printing
    mwcu::BlobPosition position(1, 2);
    {
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Position Comparison Test");
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Section Breathing Test");

    {
        PVV("Default constructor");
        mwcu::BlobSection obj;
        ASSERT_EQ(obj.start(), mwcu::BlobPosition());
        ASSERT_EQ(obj.end(), mwcu::BlobPosition());
        obj.start() = mwcu::BlobPosition(1, 2);
        obj.end()   = mwcu::BlobPosition(9, 8);
        ASSERT_EQ(obj.start(), mwcu::BlobPosition(1, 2));
        ASSERT_EQ(obj.end(), mwcu::BlobPosition(9, 8));
    }

    {
        PVV("Initialized constructor");
        mwcu::BlobSection obj(mwcu::BlobPosition(1, 2),
                              mwcu::BlobPosition(9, 8));
        ASSERT_EQ(obj.start(), mwcu::BlobPosition(1, 2));
        ASSERT_EQ(obj.end(), mwcu::BlobPosition(9, 8));
        obj.start() = mwcu::BlobPosition(3, 4);
        obj.end()   = mwcu::BlobPosition(8, 9);
        ASSERT_EQ(obj.start(), mwcu::BlobPosition(3, 4));
        ASSERT_EQ(obj.end(), mwcu::BlobPosition(8, 9));
    }

    // Verifying printing
    mwcu::BlobSection obj(mwcu::BlobPosition(1, 2), mwcu::BlobPosition(9, 8));
    {
        mwcu::MemOutStream os(s_allocator_p);
        os << obj;
        PV("section = " << os.str());
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Buffer Size Test");
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Is Valid Pos Test");

    bdlbb::Blob blob(s_allocator_p);
    mwctst::BlobTestUtil::fromString(&blob, "ab|cde|f|ghiXX", s_allocator_p);

    struct Test {
        int                d_line;
        mwcu::BlobPosition d_position;
        bool               d_expected;  // expected result
    } k_DATA[] = {
        // All valid positions
        {L_, mwcu::BlobPosition(0, 0), true},
        {L_, mwcu::BlobPosition(0, 1), true},
        {L_, mwcu::BlobPosition(1, 0), true},
        {L_, mwcu::BlobPosition(1, 1), true},
        {L_, mwcu::BlobPosition(1, 2), true},
        {L_, mwcu::BlobPosition(2, 0), true},
        {L_, mwcu::BlobPosition(3, 0), true},
        {L_, mwcu::BlobPosition(3, 1), true},
        {L_, mwcu::BlobPosition(3, 2), true},
        {L_, mwcu::BlobPosition(4, 0), true},  // One past the end
        // Invalid positions
        {L_, mwcu::BlobPosition(0, 2), false},
        {L_, mwcu::BlobPosition(0, 3), false},
        {L_, mwcu::BlobPosition(1, 3), false},
        {L_, mwcu::BlobPosition(3, 3), false},
        {L_, mwcu::BlobPosition(4, 1), false},
        {L_, mwcu::BlobPosition(5, 0), false},
        {L_, mwcu::BlobPosition(5, 1), false},
    };
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        ASSERT_EQ_D("line " << test.d_line,
                    mwcu::BlobUtil::isValidPos(blob, test.d_position),
                    test.d_expected);
    }
}
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Position To Offset Test");
    ...
    struct Test {
        ...
        // All valid positions
        {L_, mwcu::BlobPosition(0, 0), 0},
        {L_, mwcu::BlobPosition(0, 1), 1},
        {L_, mwcu::BlobPosition(1, 0), 2},
        {L_, mwcu::BlobPosition(1, 1), 3},
        {L_, mwcu::BlobPosition(1, 2), 4},
        {L_, mwcu::BlobPosition(2, 0), 5},
        {L_, mwcu::BlobPosition(3, 0), 6},
        {L_, mwcu::BlobPosition(3, 1), 7},
        {L_, mwcu::BlobPosition(3, 2), 8},
        {L_, mwcu::BlobPosition(4, 0), 9},  // One past the end
        // Invalid positions
        {L_, mwcu::BlobPosition(0, 2), -1},
        {L_, mwcu::BlobPosition(0, 3), -1},
        {L_, mwcu::BlobPosition(1, 3), -1},
        {L_, mwcu::BlobPosition(3, 3), -1},
        {L_, mwcu::BlobPosition(4, 1), -1},
        {L_, mwcu::BlobPosition(5, 0), -1},
        {L_, mwcu::BlobPosition(5, 1), -1},
    };
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        int offset = -1;
        int rc     = mwcu::BlobUtil::positionToOffsetSafe(&offset,
                                                      blob,
                                                      test.d_position);
        ...
        if (test.d_expected >= 0) {
            ...
            offset = -1;
            mwcu::BlobUtil::positionToOffset(&offset, blob, test.d_position);
            ASSERT_EQ_D("line " << test.d_line, offset, test.d_expected);
        }
        else {
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Find Offset Test");
    ...
    PV("Without a start offset");
    {
        struct Test {
            ...
            int                d_offset;
            int                d_expectedRc;
            mwcu::BlobPosition d_expected;
        } k_DATA[] = {
            // Valid offsets
            {L_, 0, 0, mwcu::BlobPosition(0, 0)},
            {L_, 1, 0, mwcu::BlobPosition(0, 1)},
            {L_, 2, 0, mwcu::BlobPosition(1, 0)},
            {L_, 3, 0, mwcu::BlobPosition(1, 1)},
            {L_, 4, 0, mwcu::BlobPosition(1, 2)},
            {L_, 5, 0, mwcu::BlobPosition(2, 0)},
            {L_, 6, 0, mwcu::BlobPosition(3, 0)},
            {L_, 7, 0, mwcu::BlobPosition(3, 1)},
            {L_, 8, 0, mwcu::BlobPosition(3, 2)},
            {L_, 9, 0, mwcu::BlobPosition(4, 0)},
            // Invalid offsets
            {L_, 10, -3, mwcu::BlobPosition(0, 0)},
            {L_, 11, -3, mwcu::BlobPosition(0, 0)},
        };
        ...
        for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
            ...
            if (test.d_expectedRc == 0) {
                ...
                rc = mwcu::BlobUtil::findOffset(&position,
                                                blob,
                                                test.d_offset);
                ASSERT_EQ_D("line " << test.d_line, rc, test.d_expectedRc);
                ASSERT_EQ_D("line " << test.d_line, position, test.d_expected);
    ...
    PV("Using a start offset");
    {
        struct Test {
            int                d_line;
            mwcu::BlobPosition d_start;
            int                d_offset;
            int                d_expectedRc;
            mwcu::BlobPosition d_expected;
        } k_DATA[] = {
            // Start of (0, 1)
            {L_, mwcu::BlobPosition(0, 1), 0, 0, mwcu::BlobPosition(0, 1)},
            {L_, mwcu::BlobPosition(0, 1), 1, 0, mwcu::BlobPosition(1, 0)},
            {L_, mwcu::BlobPosition(0, 1), 2, 0, mwcu::BlobPosition(1, 1)},
            {L_, mwcu::BlobPosition(0, 1), 3, 0, mwcu::BlobPosition(1, 2)},
            {L_, mwcu::BlobPosition(0, 1), 4, 0, mwcu::BlobPosition(2, 0)},
            {L_, mwcu::BlobPosition(0, 1), 5, 0, mwcu::BlobPosition(3, 0)},
            {L_, mwcu::BlobPosition(0, 1), 6, 0, mwcu::BlobPosition(3, 1)},
            {L_, mwcu::BlobPosition(0, 1), 7, 0, mwcu::BlobPosition(3, 2)},
            {L_, mwcu::BlobPosition(0, 1), 8, 0, mwcu::BlobPosition(4, 0)},
            {L_, mwcu::BlobPosition(0, 1), 9, -3, mwcu::BlobPosition(0, 0)},
            {L_, mwcu::BlobPosition(0, 1), 10, -3, mwcu::BlobPosition(0, 0)},
            // Start at (1,0)
            {L_, mwcu::BlobPosition(1, 0), 0, 0, mwcu::BlobPosition(1, 0)},
            {L_, mwcu::BlobPosition(1, 0), 1, 0, mwcu::BlobPosition(1, 1)},
            {L_, mwcu::BlobPosition(1, 0), 2, 0, mwcu::BlobPosition(1, 2)},
            {L_, mwcu::BlobPosition(1, 0), 3, 0, mwcu::BlobPosition(2, 0)},
            {L_, mwcu::BlobPosition(1, 0), 4, 0, mwcu::BlobPosition(3, 0)},
            {L_, mwcu::BlobPosition(1, 0), 5, 0, mwcu::BlobPosition(3, 1)},
            {L_, mwcu::BlobPosition(1, 0), 6, 0, mwcu::BlobPosition(3, 2)},
            {L_, mwcu::BlobPosition(1, 0), 7, 0, mwcu::BlobPosition(4, 0)},
            {L_, mwcu::BlobPosition(1, 0), 8, -3, mwcu::BlobPosition(0, 0)},
            {L_, mwcu::BlobPosition(1, 0), 9, -3, mwcu::BlobPosition(0, 0)},
            // Start at (1, 2)
            {L_, mwcu::BlobPosition(1, 2), 0, 0, mwcu::BlobPosition(1, 2)},
            {L_, mwcu::BlobPosition(1, 2), 1, 0, mwcu::BlobPosition(2, 0)},
            {L_, mwcu::BlobPosition(1, 2), 2, 0, mwcu::BlobPosition(3, 0)},
            {L_, mwcu::BlobPosition(1, 2), 3, 0, mwcu::BlobPosition(3, 1)},
            {L_, mwcu::BlobPosition(1, 2), 4, 0, mwcu::BlobPosition(3, 2)},
            {L_, mwcu::BlobPosition(1, 2), 5, 0, mwcu::BlobPosition(4, 0)},
            {L_, mwcu::BlobPosition(1, 2), 6, -3, mwcu::BlobPosition(0, 0)},
            {L_, mwcu::BlobPosition(1, 2), 7, -3, mwcu::BlobPosition(0, 0)},
            // Start at (3, 2)
            {L_, mwcu::BlobPosition(3, 2), 0, 0, mwcu::BlobPosition(3, 2)},
            {L_, mwcu::BlobPosition(3, 2), 1, 0, mwcu::BlobPosition(4, 0)},
            {L_, mwcu::BlobPosition(3, 2), 2, -3, mwcu::BlobPosition(0, 0)},
            {L_, mwcu::BlobPosition(3, 2), 3, -3, mwcu::BlobPosition(0, 0)},
            // Start at (4, 0) (i.e. one past the end)
            {L_, mwcu::BlobPosition(4, 0), 0, 0, mwcu::BlobPosition(4, 0)},
            {L_, mwcu::BlobPosition(4, 0), 1, -2, mwcu::BlobPosition(0, 0)},
            // Invalid start
            {L_, mwcu::BlobPosition(5, 2), 0, -1, mwcu::BlobPosition(0, 0)},
            {L_, mwcu::BlobPosition(5, 2), 1, -1, mwcu::BlobPosition(0, 0)}};
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Reserve Test");
    ...
    struct Test {
        ...
        {L_, "", 1, mwcu::BlobPosition(0, 0)},
        {L_, "a", 3, mwcu::BlobPosition(1, 0)},
        {L_, "abcd", 3, mwcu::BlobPosition(1, 0)},
        {L_, "abcd|e", 3, mwcu::BlobPosition(2, 0)},
        {L_, "abcd|efg", 3, mwcu::BlobPosition(2, 0)},
        {L_, "abcd|efgX", 1, mwcu::BlobPosition(1, 3)},
        {L_, "abcd|efgX", 2, mwcu::BlobPosition(1, 3)},
    };
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        PVVV("reserve");
        {
            ...
            mwcu::BlobUtil::reserve(&blob, test.d_reserveLength);
            ASSERT_EQ_D("line " << test.d_line,
                        blob.length(),
        ...
        PVVV("reserve and load the position");
        {
            ...
            mwcu::BlobUtil::reserve(&pos, &blob, test.d_reserveLength);
            ASSERT_EQ_D("line " << test.d_line,
                        blob.length(),
                        length + test.d_reserveLength);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Write Bytes Test");
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        int rc = mwcu::BlobUtil::writeBytes(&blob,
                                            test.d_position,
                                            test.d_data,
        ...
        mwctst::BlobTestUtil::toString(&resultBlob, blob);
        ASSERT_EQ_D("line " << test.d_line,
                    resultBlob,
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Append Blob From Index Test");
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        mwctst::BlobTestUtil::fromString(&src, test.d_data, s_allocator_p);
        ...
        bsl::string resultBlob(s_allocator_p);
        mwctst::BlobTestUtil::toString(&resultBlob, blob, true);
        ASSERT_EQ_D("line " << test.d_line,
                    resultBlob,
                    bsl::string(test.d_resultBlob, s_allocator_p));
    }
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Section Size Test");
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        int               size = 0;
        const int rc = mwcu::BlobUtil::sectionSize(&size, blob, section);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Compare Section Test");
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        int       cmpResult = 0;
        const int rc        = mwcu::BlobUtil::compareSection(&cmpResult,
                                                      blob,
                                                      test.d_pos,
                                                      test.d_data,
                                                      test.d_length);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Copy To Raw Buffer From Index Test");
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        mwcu::BlobUtil::copyToRawBufferFromIndex(dst + test.d_rawBufferOffset,
                                                 blob,
                                                 test.d_index,
                                                 test.d_byte,
                                                 test.d_length);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Read Up To N Bytes Test");
    ...
    struct Test {
        ...
        {L_, mwcu::BlobPosition(0, 0), 0, 0, ""},
        {L_, mwcu::BlobPosition(0, 0), 1, 1, "a"},
        {L_, mwcu::BlobPosition(0, 0), 2, 2, "ab"},
        {L_, mwcu::BlobPosition(0, 0), 3, 3, "abc"},
        {L_, mwcu::BlobPosition(0, 0), 4, 4, "abcd"},
        {L_, mwcu::BlobPosition(0, 0), 5, 5, "abcde"},
        {L_, mwcu::BlobPosition(0, 0), 6, 6, "abcdef"},
        {L_, mwcu::BlobPosition(0, 0), 7, 7, "abcdefg"},
        {L_, mwcu::BlobPosition(0, 0), 8, 8, "abcdefgh"},
        {L_, mwcu::BlobPosition(0, 0), 9, 9, "abcdefghi"},
        {L_, mwcu::BlobPosition(0, 0), 10, 9, "abcdefghi"},
        {L_, mwcu::BlobPosition(0, 1), 1, 1, "b"},
        {L_, mwcu::BlobPosition(0, 1), 5, 5, "bcdef"},
        {L_, mwcu::BlobPosition(2, 0), 3, 3, "fgh"},
        {L_, mwcu::BlobPosition(3, 0), 0, 0, ""},
        {L_, mwcu::BlobPosition(3, 0), 1, 1, "g"},
        {L_, mwcu::BlobPosition(3, 0), 2, 2, "gh"},
        {L_, mwcu::BlobPosition(3, 0), 3, 3, "ghi"},
        {L_, mwcu::BlobPosition(3, 0), 12, 3, "ghi"},
        {L_, mwcu::BlobPosition(4, 0), 0, 0, ""},
        {L_, mwcu::BlobPosition(4, 0), 1, -2, ""},  // invalid length
        {L_, mwcu::BlobPosition(5, 1), 0, -1, ""},  // invalid start
    };
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        int rc = mwcu::BlobUtil::readUpToNBytes(buffer,
                                                blob,
                                                test.d_start,
                                                test.d_length);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Read N Bytes Test");
    ...
    struct Test {
        ...
        {L_, mwcu::BlobPosition(0, 0), 0, 0, ""},
        {L_, mwcu::BlobPosition(0, 0), 1, 0, "a"},
        {L_, mwcu::BlobPosition(0, 0), 2, 0, "ab"},
        {L_, mwcu::BlobPosition(0, 0), 3, 0, "abc"},
        {L_, mwcu::BlobPosition(0, 0), 4, 0, "abcd"},
        {L_, mwcu::BlobPosition(0, 0), 5, 0, "abcde"},
        {L_, mwcu::BlobPosition(0, 0), 6, 0, "abcdef"},
        {L_, mwcu::BlobPosition(0, 0), 7, 0, "abcdefg"},
        {L_, mwcu::BlobPosition(0, 0), 8, 0, "abcdefgh"},
        {L_, mwcu::BlobPosition(0, 0), 9, 0, "abcdefghi"},
        {L_, mwcu::BlobPosition(0, 0), 10, -2, "abcdefghi"},  // invalid end
        {L_, mwcu::BlobPosition(0, 1), 1, 0, "b"},
        {L_, mwcu::BlobPosition(0, 1), 5, 0, "bcdef"},
        {L_, mwcu::BlobPosition(2, 0), 3, 0, "fgh"},
        {L_, mwcu::BlobPosition(3, 0), 0, 0, ""},
        {L_, mwcu::BlobPosition(3, 0), 1, 0, "g"},
        {L_, mwcu::BlobPosition(3, 0), 2, 0, "gh"},
        {L_, mwcu::BlobPosition(3, 0), 3, 0, "ghi"},
        {L_, mwcu::BlobPosition(3, 0), 12, -2, "ghi"},
        {L_, mwcu::BlobPosition(4, 0), 0, 0, ""},
        {L_, mwcu::BlobPosition(4, 0), 1, -21, ""},
        // invalid length
        {L_, mwcu::BlobPosition(5, 1), 0, -11, ""},
        // invalid start
    };
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        int rc = mwcu::BlobUtil::readNBytes(buffer,
                                            blob,
                                            test.d_start,
                                            test.d_length);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Append To Blob Test");
...

    {
        ...
        for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
            ...
            bdlbb::Blob src(s_allocator_p);
            mwctst::BlobTestUtil::fromString(&src,
                                             test.d_srcPattern,
                                             s_allocator_p);
            ...
            const int rc =
                mwcu::BlobUtil::appendToBlob(&blob, src, start, test.d_length);
...

    {
        ...
        for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
            ...
            const int rc = mwcu::BlobUtil::appendToBlob(&blob, src, start);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Get Aligned Section Test");
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        char        storageSafe[10];
        const char* resSafe = mwcu::BlobUtil::getAlignedSectionSafe(
            storageSafe,
            blob,
            start,
            test.d_length,
        ...
        char        storage[10];
        const char* res = mwcu::BlobUtil::getAlignedSection(
            storage,
            blob,
            start,
            test.d_length,
        ...
        char      expected[10];
        const int rc =
            mwcu::BlobUtil::readNBytes(expected, blob, start, test.d_length);
        ASSERT_EQ_D("line " << test.d_line, rc, 0);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Get Aligned Object Test");
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        TestObject* resSafe     = mwcu::BlobUtil::getAlignedObjectSafe(
            &storageSafe,
            blob,
            start,
            test.d_copyFromBlob);
        ...
        TestObject* res     = mwcu::BlobUtil::getAlignedObject(
            &storage,
            blob,
            start,
            test.d_copyFromBlob);
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("Blob Start Hex Dumper Test");

    {
        ...
        os << mwcu::BlobStartHexDumper(&blob);
        ASSERT_EQ(os.str(), "");
    }

    {
        ...
        os << mwcu::BlobStartHexDumper(&blob, 8);
        ASSERT_EQ(os.str(),
                  "     0:   "
                  "61626364 65666768                       "
                  "|abcdefgh        |\n");
...

    {
        ...
        os << mwcu::BlobStartHexDumper(&blob, 6);
        ASSERT_EQ(os.str(),
                  "     0:   "
                  "61626364 6566                           "
                  "|abcdef          |\n"
