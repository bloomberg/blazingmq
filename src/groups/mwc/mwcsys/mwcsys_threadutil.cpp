...
namespace {
const char k_LOG_CATEGORY[] = "MWCSYS.THREADUTIL";
...
void ThreadUtil::setCurrentThreadName(const bsl::string& value)
{
    ...
    if (rc != 0) {
        ...
        BALL_LOG_ERROR << "Failed to set thread name "
                       << "[name: '" << value << "'"
                       << ", rc: " << rc << ", strerr: '" << bsl::strerror(rc)
                       << "']";
    }
}
...
void ThreadUtil::setCurrentThreadNameOnce(const bsl::string& value)
{
    ...
    [[clang::no_destroy]]
#endif
    static mwcu::TLSBool s_named(false, true);
