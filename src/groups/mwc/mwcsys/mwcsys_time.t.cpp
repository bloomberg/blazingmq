...
//
// Plan:
//   - Create a TestClock, initialize the mwcsys::TimeUtil with it, and
...
// ------------------------------------------------------------------------
{
    ...
    PV("Testing real time clock");
    {
        ...
        ASSERT_EQ(bsls::TimeInterval(5, 0), mwcsys::Time::nowRealtimeClock());

        testClock.d_realtimeClock.setTotalSeconds(0);
        ASSERT_EQ(bsls::TimeInterval(0, 0), mwcsys::Time::nowRealtimeClock());
    }

    PV("Testing monotonic clock");
    {
        testClock.d_monotonicClock.setTotalSeconds(5);
        ASSERT_EQ(bsls::TimeInterval(5, 0), mwcsys::Time::nowMonotonicClock());

        testClock.d_monotonicClock.setTotalSeconds(0);
        ASSERT_EQ(bsls::TimeInterval(0, 0), mwcsys::Time::nowMonotonicClock());
    }

    PV("Testing high resolution timer");
    {
        testClock.d_highResTimer = 1000000;
        ASSERT_EQ(1000000, mwcsys::Time::highResolutionTimer());

        testClock.d_highResTimer = 0;
        ASSERT_EQ(0, mwcsys::Time::highResolutionTimer());
    }
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("DEFAULT INITIALIZE & SHUTDOWN");

    // Initialize the 'Time'.
    ASSERT_SAFE_PASS(mwcsys::Time::initialize());

    // Initialize should be a no-op.
    ASSERT_SAFE_PASS(mwcsys::Time::initialize());

    // Shutdown the Time is a no-op.
    ASSERT_SAFE_PASS(mwcsys::Time::shutdown());

    // Shutdown the Time.
    ASSERT_SAFE_PASS(mwcsys::Time::shutdown());

    // Shutdown again should assert
    ASSERT_SAFE_FAIL(mwcsys::Time::shutdown());
}
...
// ------------------------------------------------------------------------
{
    mwctst::TestHelper::printTestName("CUSTOM INITIALIZE & SHUTDOWN");
...
static void testN1_defaultPerformance_GoogleBenchmark(benchmark::State& state)
{
    ...
    mwctst::TestHelper::printTestName("PERFORMANCE GOOGLE BENCHMARK");
