...
// mwcst_testutil.h -*-C++-*-
#ifndef INCLUDED_MWCST_TESTUTIL
#define INCLUDED_MWCST_TESTUTIL
...
#define ASSERT_EQUALS_SKIP(X, Y, SKIP)                                        \
    {                                                                         \
        if (!((X) == (Y))) {                                                  \
            bsl::cout << "Error " << __FILE__ << "(" << __LINE__              \
                      << "): " << #X << " (" << mwcu::PrintUtil::printer(X)   \
                      << ") == " << #Y << " (" << mwcu::PrintUtil::printer(Y) \
                      << ")"                                                  \
                      << "    (failed)" << bsl::endl;                         \
            if ((SKIP))                                                       \
                testStatus = 254;                                             \
...
#define LOOP_ASSERT_EQUALS_SKIP(I, X, Y, SKIP)                                \
    {                                                                         \
        if (!((X) == (Y))) {                                                  \
            cout << #I << ": " << mwcu::PrintUtil::printer(I) << "\n"         \
                 << "Error " << __FILE__ << "(" << __LINE__ << "): " << #X    \
                 << " (" << mwcu::PrintUtil::printer(X) << ") == " << #Y      \
                 << " (" << mwcu::PrintUtil::printer(Y) << ")"                \
                 << "    (failed)" << endl;                                   \
            if ((SKIP))                                                       \
                testStatus = 254;                                             \
            else if (testStatus >= 0 && testStatus <= 100)                    \
