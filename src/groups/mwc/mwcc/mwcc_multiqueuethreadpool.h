...
// mwcc_multiqueuethreadpool.h                                        -*-C++-*-
#ifndef INCLUDED_MWCC_MULTIQUEUETHREADPOOL
#define INCLUDED_MWCC_MULTIQUEUETHREADPOOL

//@PURPOSE: Provide a set of monitored queues processed by a thread pool.
// MultiQueueThreadPool is referred to as MQTP below.
//
//@CLASSES:
//  mwcc::MultiQueueThreadPool: Queues processed by a thread pool
//  mwcc::MultiQueueThreadPoolConfig: Configuration for a MQTP
//  mwcc::MultiQueueThreadPoolUtil: Utilities for MQTP
//  mwcc::MultiQueueThreadPool_QueueCreatorRet: MQTP queueCreator args
//
//@DESCRIPTION: This component defines a mechanism,
// 'mwcc::MultiQueueThreadPool', which encapsulates the common pattern of
...
// common use case, the 'mwcc::MultiQueueThreadPool' offers additional options
// for its operation:
// - Creating it with a 'bdlmt::EventScheduler' allows the
//   'mwcc::MultiQueueThreadPool' to enqueue items on the appropriate queue at
//   the requested time.
// - Creating the 'mwcc::MultiQueueThreadPool' without a 'bdlmt::ThreadPool'
...
//..
//  typedef MultiQueueThreadPool< mwcc::MonitoredFixedQueue<int> > MQTP;
//..
...
//  {
//      if (event->type() == MQTP::Event::MWCC_USER) {
...
//  }
//..
// Notice that the handler first checks the type of event.  'MWCC_USER'
...
class MultiQueueThreadPoolEvent {
  public:
    // PUBLIC TYPES
    enum Type {
        MWCC_USER  // Event generated by the user.
        ,
        MWCC_FINALIZE_EVENT  // For events that were enqueued on multiple
        ...
        ,
        MWCC_QUEUE_EMPTY  // Automatically generated when no more items are
...
class MultiQueueThreadPool_QueueCreatorRet {
  ...
  public:
    ...
    /// Create a new `mwcc::MultiQueueThreadPool_QueueCreatorRet` object
...
class MultiQueueThreadPoolConfig {
  public:
    ...
    enum EventFinalizationType {
        // Enum controlling for which events 'MWCC_FINALIZE_EVENT' events are
        // generated.

        MWCC_FINALIZE_NONE  // Don't finalize any events
        ,
        MWCC_FINALIZE_MULTI_QUEUE  // Finalize only events that were enqueued
                                   // on multiple queues
        ,
        MWCC_FINALIZE_ALL  // Finalize all events
    };
  ...
  public:
    ...
    /// Create a `mwcc::MultiQueueThreadPoolConfig` for a
    ...
    /// Create a `mwcc::MultiQueueThreadPoolConfig` object having the same
    ...
    /// Set for which types of events `MWCC_FINALIZE_EVENT` events should be
    /// generated and return a reference offering modifiable access to this
    /// object.  By default, this is `MWCC_FINALIZE_NONE`.
    MultiQueueThreadPoolConfig<TYPE>&
...
class MultiQueueThreadPool BSLS_KEYWORD_FINAL {
  ...
  private:
    ...
    BALL_LOG_SET_CLASS_CATEGORY("MWCC.MULTIQUEUETHREADPOOL");
  ...
  public:
    ...
    /// Create a `mwcc::MultiQueueThreadPool` with the specified `config`
    ...
    /// `mwcc::MultiQueueThreadPool` was created without a
    ...
    while (true) {
        ...
        if (unrefEvent(event, false)) {
            bool finalize = (d_config.d_finalizeType ==
                             Config::MWCC_FINALIZE_ALL);
            finalize      = finalize || ((d_config.d_finalizeType ==
                                     Config::MWCC_FINALIZE_MULTI_QUEUE) &&
                                    event->d_enqueuedOnMultipleQueues);

            if (finalize) {
                event->d_type = Event::MWCC_FINALIZE_EVENT;
                d_config.d_eventCallbackFn(queue,
                                           info.d_context_p.get(),
                                           event);
            }
...
{
    d_queueEmptyEvent.d_type = Event::MWCC_QUEUE_EMPTY;
}
