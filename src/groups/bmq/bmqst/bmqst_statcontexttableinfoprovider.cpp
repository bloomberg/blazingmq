...
// bmqst_statcontexttableinfoprovider.cpp                             -*-C++-*-
                 ...
                 StatContext::ValueType valueType) const BSLS_KEYWORD_OVERRIDE
    {
        ...
        if (valueType == StatContext::e_TOTAL_VALUE) {
            if (context.hasName()) {
                ...
                length = bmqst::PrintUtil::printedValueLength(context.id());
            }
        }
    ...
    if (colInfo.d_customColumn_p) {
        ...
        if (colInfo.d_intFunc) {
            ...
            if (funcValue == MAX_INT || funcValue == MIN_INT) {
                ...
                if (d_printSeparators) {
                    bmqst::PrintUtil::printValueWithSeparator(stream,
                                                              funcValue,
                                                              3,
                                                              ',');
                }
                ...
                bmqst::PrintUtil::printTimeIntervalNs(stream,
                                                      funcValue,
                                                      d_precision);
            }
        }
