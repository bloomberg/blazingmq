...
// bmqst_table.h                                                      -*-C++-*-
#ifndef INCLUDED_BMQST_TABLE
#define INCLUDED_BMQST_TABLE

//@PURPOSE: Provide a simple table of values derived from a table stat context.
//
//@CLASSES:
// bmqst::Table : table of values backed by a 'bmqst::StatContext'.
//
//@SEE_ALSO: bmqst_statcontext
//           bmqst_tableschema
//           bmqst_tablerecord
//
//@DESCRIPTION: This component defines a mechanism, 'bmqst::Table', which is a
// table of values derived from a table 'bmqst::StatContext'.
//
// A 'bmqst::Table' must be associated with a 'bmqst::StatContext' after
...
// 'bmqst::TableSchema' and the records are defined by a 'bmqst::TableRecords'.
//
// The application defines the schema of a table by adding named columns that
// are defined using functions (from 'bmqst::StatUtil') and their arguments.
...
// it on a regular basis.  The application creates a 'bmqst::StatContext' with
// two 'bmqst::Value': "IN" representing the input traffic, and "OUT"
// representing the output traffic.  The stat context can be initialized using
// a 'bmqst::StatContextConfiguration' created by the following function:
//..
//  bmqst::StatContextConfiguration
//  createStatContextConfiguration(int               numSamples,
//                                 bslma::Allocator *allocator)
//  {
//       bmqst::StatContextConfiguration config("Total", allocator);
//       config.isTable(true);
...
//  }
//..
// To print the stat context, the application first creates a 'bmqst::Table'
// and initializes it using the following function:
//..
//  void initTable(bmqst::Table       *table,
//                 bmqst::StatContext *statContext,
//                 int                 numSamples)
//  {
//      typedef bmqst::StatUtil SU;
//      typedef bmqst::StatValue SV;
//
//      int inValueIndex = 0;
//      int outValueIndex = 1;
//
...
// 'bmqst::BasicTableInfoProvider' associated with the table (pass the table
// in the constructor, and initializing it using the following function:
//..
//  void initTIP(bmqst::BasicTableInfoProvider *tip,
//               int                            numSamples)
//  {
//      // ID
//      tip->setColumnGroup("");
...
// in 'bmqst::TableUtil' for details.  The table that will be printed looks
