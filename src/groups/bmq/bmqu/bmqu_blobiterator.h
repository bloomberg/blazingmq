...
// bmqu_blobiterator.h                                                -*-C++-*-
#ifndef INCLUDED_BMQU_BLOBITERATOR
#define INCLUDED_BMQU_BLOBITERATOR

//@PURPOSE: Provide an iterator over a section of a blob
//
//@CLASSES:
// bmqu::BlobIterator: a blob iterator
//
//@DESCRIPTION:
// This component defines a mechanism, 'bmqu::BlobIterator', useful for
...
class BlobIterator {
  ...
  private:
    ...
    bmqu::BlobPosition d_pos;  // current position
...
{
    int rc = bmqu::BlobUtil::findOffsetSafe(&d_pos, *d_blob_p, d_pos, length);
    if (rc != 0) {
        d_remaining = -1;
        return false;  // RETURN
    }
...
{
    int rc = bmqu::BlobUtil::findOffset(&d_pos, *d_blob_p, d_pos, length);
    if (rc != 0) {
        d_remaining = -1;
        return false;  // RETURN
    }
...
inline const bmqu::BlobPosition& BlobIterator::position() const
{
    return d_pos;
}
