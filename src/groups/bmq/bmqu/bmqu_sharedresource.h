...
// bmqu_sharedresource.h                                              -*-C++-*-
#ifndef INCLUDED_BMQU_SHAREDRESOURCE
#define INCLUDED_BMQU_SHAREDRESOURCE

//@PURPOSE: Provides a mechanism to manage the lifetime of a shared resource.
//
//@CLASSES:
//  bmqu::SharedResource: a shared resource
//
//@SEE ALSO:
//  bmqu_weakmemfn
//
//@DESCRIPTION:
// This component provides a utility mechanism 'bmqu::SharedResource' used to
// manage the lifetime of a shared resource.
//
// The way this works is that a 'bmqu::SharedResource<R>' holds a shared
...
// 'bmqu::SharedResource' to free the resource upon invalidation.
//
// The first one, used by default, is 'bmqu::SharedResourceNoOpDeleter'.  As
// the name suggests this deleter does nothing to the managed resource.
//
// The second one, 'bmqu::SharedResourceFactoryDeleter', is a wrapper around a
...
//
/// Comparison to 'bmqu::AtomicValidator'
///-------------------------------------
// This component is intended as a replacement for 'bmqu_atomicvalidator'.
...
//  {
//     bmqu::AtomicValidatorGuard guard(validator.ptr());
//     if (!guard.isValid()) {
...
//  }
//..
// 'bmqu::SharedResource', on the other hand, is less intrusive, as will be
...
// functions, 'bmqu::SharedResource' is fully thread-safe, meaning that
...
//
// For that we'll need a 'bmqu::SharedResource<MyService>' object held as a
...
// same way it does with shared pointers.  Helpfully, the 'bmqu' package
// provides an utility component 'bmqu_weakmemfn', that can help us with that.
//..
//  void
//  MyService::asyncDoStuff()
//  {
//      // acquire the shared resource and bind to the async operation
//      d_threadPool_p->enqueueJob(bmqu::WeakMemFnUtil::weakMemFn(
//                                                      &MyService::doStuff,
//                                                      d_self.acquireWeak()));
//  }
//..
