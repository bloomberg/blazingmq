...
// bmqio_ntcchannel.h                                                 -*-C++-*-
#ifndef INCLUDED_BMQIO_NTCCHANNEL
#define INCLUDED_BMQIO_NTCCHANNEL

//@PURPOSE: Provide a bi-directional async channel implemented using NTC.
//
//@CLASSES:
//  bmqio::Channel: Interface for a NTC-based bi-directional async channel.
//
//@DESCRIPTION: This component provides a mechanism, 'bmqio::NtcChannel',
...
class NtcRead {
  ...
  public:
    ...
    NtcRead(const bmqio::Channel::ReadCallback& callback,
            int                                 numNeeded,
    ...
    const bmqio::Channel::ReadCallback& callback() const;
...
class NtcReadQueue {
    ...
    typedef bsl::list<bsl::shared_ptr<bmqio::NtcRead> > List;
  ...
  public:
    ...
    void append(const bsl::shared_ptr<bmqio::NtcRead>& operation);

    /// Remove the specified `operation` from the queue.
    void remove(const bsl::shared_ptr<bmqio::NtcRead>& operation);
    ...
    /// `operation`.
    void pop(bsl::shared_ptr<bmqio::NtcRead>* operation);

    /// Return the front of the queue.
    bsl::shared_ptr<bmqio::NtcRead> front();
...
class NtcChannel : public bmqio::Channel,
  ...
  private:
    ...
    void processReadTimeout(const bsl::shared_ptr<bmqio::NtcRead>& read,
                            const bsl::shared_ptr<ntci::Timer>&    timer,
                            const ntca::TimerEvent&                event);

    /// Process the cancellation of the specified `read`.
    void processReadCancelled(const bsl::shared_ptr<bmqio::NtcRead>& read);
  ...
  public:
    ...
    int connect(bmqio::Status* status, const bmqio::ConnectOptions& options);
    ...
    /// enumerators in `bmqio_ChannelStatus::Type`.  Note that success does
    ...
    /// with a `bmqio::ChannelStatus::e_CANCELED` status.  Note that if the
...
class NtcListener : public bmqio::ChannelFactoryOperationHandle,
  ...
  public:
    ...
    /// listener is closed.
    typedef void CloseFnType(const bmqio::Status& status);
