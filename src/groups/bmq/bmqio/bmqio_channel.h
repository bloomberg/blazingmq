...
// bmqio_channel.h                                                    -*-C++-*-
#ifndef INCLUDED_BMQIO_CHANNEL
#define INCLUDED_BMQIO_CHANNEL

//@PURPOSE: Provide a pure protocol for a bi-directional async channel.
//
//@CLASSES:
//  bmqio::Channel: Interface for a bi-directional async channel.
//
//@DESCRIPTION: This component provides a pure protocol, 'bmqio::Channel', to
// be implemented by any transport mechanism to asynchronously send and receive
// arbitrary blobs of data.
//
// A 'bmqio::Channel' provides operations to asynchronously read and write data
// (e.g. blobs).
...
class Channel {
  public:
    ...
    /// callback is invoked with three arguments: (1) a `bmqio::Status` that
    /// indicates the result of the read operation, and if this status is
    /// equal to `BMQIO_SUCCESS`, (2) a pointer to an integer value where
