...
// bmqsys_executil.h                                                  -*-C++-*-
#ifndef INCLUDED_BMQSYS_EXECUTIL
#define INCLUDED_BMQSYS_EXECUTIL

//@PURPOSE: Provide utilities to execute commands on the system.
//
//@CLASSES:
//  bmqsys::ExecUtil: Utility to execute commands on the system
//
//@DESCRIPTION: 'bmqsys::ExecUtil' provides a utility namespace for executing
...
//  // Execute the command
//  bsl::string output;
//  int rc = bmqsys::ExecUtil::execute(&output, command.str().c_str());
//  if (rc != 0) {
