...
// bmqsys_executil.t.cpp                                              -*-C++-*-
#include <bmqsys_executil.h>
...
static void createFile(const char* path, const char* content, mode_t mode)
{
    ...
    int numBytes = bsl::strlen(content);
    ASSERT_EQ(bdls::FilesystemUtil::write(fd, content, numBytes), numBytes);
    ASSERT_EQ(bdls::FilesystemUtil::close(fd), 0);
...
// ------------------------------------------------------------------------
{
    bmqtst::TestHelper::printTestName("execute");
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        const int   rc = bmqsys::ExecUtil::execute(&output, test.d_command);
...
// ------------------------------------------------------------------------
{
    bmqtst::TestHelper::printTestName("execute (system failure)");
    ...
    rc = bmqsys::ExecUtil::execute(&output,
    ...
    rc = bmqsys::ExecUtil::execute(&output, "bash -c \"/bin/echo 'test'\"");
...
// ------------------------------------------------------------------------
{
    bmqtst::TestHelper::printTestName("outputFromFile (noGen)");

    const char k_TESTFILE[] = "./bmqsys_executil.t.3.in";
    const char k_CONTENT[]  = "Test file\ngenerated by bmqsys::ExecUtil.t";
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        bsl::string output(s_allocator_p);
        const int   rc = bmqsys::ExecUtil::outputFromFile(&output,
                                                        k_TESTFILE,
                                                        test.d_arguments);
...
// ------------------------------------------------------------------------
{
    bmqtst::TestHelper::printTestName("outputFromFile (gen)");

    const char k_TESTFILE[] = "./bmqsys_executil.t.4.in";
    ...
    for (size_t idx = 0; idx < k_NUM_DATA; ++idx) {
        ...
        ASSERT_EQ(bdls::FilesystemUtil::remove(k_TESTFILE), 0);
    }
}
...
// ------------------------------------------------------------------------
{
    bmqtst::TestHelper::printTestName("outputFromFile (error)");

    const char k_TESTFILE[] = "./bmqsys_executil.t.5.in";
