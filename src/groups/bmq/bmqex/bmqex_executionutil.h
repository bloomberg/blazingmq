...
///------------------------------------------
// Those are functions, all named 'bmqex::ExecutionUtil::thenExecute', to
...
// a 'bmqex::FutureResult' object. The callback itself may or may not return a
// value.
//..
//  // a continuation callback to be executed
//  auto myCallback = [](bmqex::FutureResult<int> result) -> bsl::string
//                    {
//                        bsl::cout << result.get() << "\n";
...
// object (the continuation callback), but also a 'bmqex::Future' object
...
///------------------------------
// 'bmqex::ExecutionUtil::invoke' is a special execution function that is
...
/// Provides a metafunction that determines, at compile time, the type
/// returned by `bmqex::ExecutionUtil::thenExecute`.
...
struct ExecutionUtil {
    ...
    /// * If `P` is a Two-Way policy - `bmqex::Future<R>`.
    template <class POLICY, class FUNCTION>
    ...
    ///:  `bmqex_executionpolicy`).
    ///:
    /// * A type `FT`, that is `bmqex::Future<R1>`.
    ...
    ///   otherwise.
    ///
    /// The type of `bmqex::ThenExecuteResult<P, FT, FN>::Type` expression
    /// is the follow:
    /// * If `P` is a One-Way policy - `void`.
    /// * If `P` is a Two-Way policy - `bmqex::Future<R2>`.
    template <class POLICY, class FUTURE, class FUNCTION>
...
ExecutionUtil_UniqueTwoWayTask<R, F>::operator()() BSLS_NOTHROW_SPEC
{
    ...
    //       of 'bmqex::FutureSharedState's 'get' function were synchronizing
