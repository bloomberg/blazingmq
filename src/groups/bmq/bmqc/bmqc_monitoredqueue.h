...
// bmqc_monitoredqueue.h                                              -*-C++-*-
#ifndef INCLUDED_BMQC_MONITOREDQUEUE
#define INCLUDED_BMQC_MONITOREDQUEUE

//@PURPOSE: Provide a queue that monitors its load.
//
//@CLASSES:
//  bmqc::MonitoredQueueState: Queue's monitor state enum
//  bmqc::MonitoredQueue:      A queue that monitors its load
//  bmqc::MonitoredQueueUtil:  Monitored  queue utilities
//
...
// 'bmqc::MonitoredQueue', which is a simple wrapper around a Queue type that
...
// is represented with the 'bmqc::MonitoredQueueState' enum.  It additionally
// defines a utility struct, 'bmqc::MonitoredQueueUtil', providing utility
...
/// This struct defines the type of state that a `bmqc::MonitoredQueue`
/// can be in.
struct MonitoredQueueState {
    // TYPES
    enum Enum {
...
class MonitoredQueue {
  ...
  public:
    ...
    /// Create a `bmqc::MonitoredQueue` object with capacity for the
...
struct MonitoredQueueUtil {
    ...
    static void stateLogCallback(const bsl::string& queueName,
                                 ...
                                 bmqc::MonitoredQueueState::Enum state);
};
...
// FREE OPERATORS
inline bsl::ostream& bmqc::operator<<(bsl::ostream&                   stream,
                                      bmqc::MonitoredQueueState::Enum value)
{
    return bmqc::MonitoredQueueState::print(stream, value, 0, -1);
}
