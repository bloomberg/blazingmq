...
const char Authenticate::CLASS_NAME[] = "Authenticate";

const bdlat_AttributeInfo Authenticate::ATTRIBUTE_INFO_ARRAY[] = {
...
const bdlat_AttributeInfo* Authenticate::lookupAttributeInfo(const char* name,
                                                             int nameLength)
{
    for (int i = 0; i < 2; ++i) {
        const bdlat_AttributeInfo& attributeInfo =
            Authenticate::ATTRIBUTE_INFO_ARRAY[i];
...
const bdlat_AttributeInfo* Authenticate::lookupAttributeInfo(int id)
{
    switch (id) {
...
Authenticate::Authenticate(bslma::Allocator* basicAllocator)
: d_mechanism(basicAllocator)
, d_data(basicAllocator)
{
}

Authenticate::Authenticate(const Authenticate& original,
                           bslma::Allocator*   basicAllocator)
...
#if defined(BSLS_COMPILERFEATURES_SUPPORT_RVALUE_REFERENCES) &&               \
    defined(BSLS_COMPILERFEATURES_SUPPORT_NOEXCEPT)
Authenticate::Authenticate(Authenticate&& original) noexcept
: d_mechanism(bsl::move(original.d_mechanism)),
  d_data(bsl::move(original.d_data))
{
}

Authenticate::Authenticate(Authenticate&&    original,
                           bslma::Allocator* basicAllocator)
...
Authenticate& Authenticate::operator=(const Authenticate& rhs)
{
    if (this != &rhs) {
...
#if defined(BSLS_COMPILERFEATURES_SUPPORT_RVALUE_REFERENCES) &&               \
    defined(BSLS_COMPILERFEATURES_SUPPORT_NOEXCEPT)
Authenticate& Authenticate::operator=(Authenticate&& rhs)
{
    if (this != &rhs) {
...
bsl::ostream&
Authenticate::print(bsl::ostream& stream, int level, int spacesPerLevel) const
{
    bslim::Printer printer(&stream, level, spacesPerLevel);
    printer.start();
...
const bdlat_SelectionInfo* ControlMessageChoice::lookupSelectionInfo(int id)
{
    ...
    case SELECTION_ID_AUTHENTICATE:
        return &SELECTION_INFO_ARRAY[SELECTION_INDEX_AUTHENTICATE];
...
, d_allocator_p(bslma::Default::allocator(basicAllocator))
{
    ...
    case SELECTION_ID_AUTHENTICATE: {
        new (d_authenticate.buffer())
            Authenticate(original.d_authenticate.object(), d_allocator_p);
    } break;
    case SELECTION_ID_AUTHENTICATE_RESPONSE: {
               ...
               d_allocator_p(original.d_allocator_p)
{
    ...
    case SELECTION_ID_AUTHENTICATE: {
        new (d_authenticate.buffer())
            Authenticate(bsl::move(original.d_authenticate.object()),
                         d_allocator_p);
    } break;
    case SELECTION_ID_AUTHENTICATE_RESPONSE: {
...
ControlMessageChoice::operator=(const ControlMessageChoice& rhs)
{
    if (this != &rhs) {
        ...
        case SELECTION_ID_AUTHENTICATE: {
            makeAuthenticate(rhs.d_authenticate.object());
        } break;
        case SELECTION_ID_AUTHENTICATE_RESPONSE: {
...
ControlMessageChoice::operator=(ControlMessageChoice&& rhs)
{
    if (this != &rhs) {
        ...
        case SELECTION_ID_AUTHENTICATE: {
            makeAuthenticate(bsl::move(rhs.d_authenticate.object()));
        } break;
        case SELECTION_ID_AUTHENTICATE_RESPONSE: {
...
void ControlMessageChoice::reset()
{
    ...
    case SELECTION_ID_AUTHENTICATE: {
        d_authenticate.object().~Authenticate();
    } break;
    case SELECTION_ID_AUTHENTICATE_RESPONSE: {
...
Authenticate& ControlMessageChoice::makeAuthenticate()
{
    if (SELECTION_ID_AUTHENTICATE == d_selectionId) {
        bdlat_ValueTypeFunctions::reset(&d_authenticate.object());
    }
    else {
        reset();
        new (d_authenticate.buffer()) Authenticate(d_allocator_p);
        d_selectionId = SELECTION_ID_AUTHENTICATE;
    }

    return d_authenticate.object();
}

Authenticate& ControlMessageChoice::makeAuthenticate(const Authenticate& value)
{
    if (SELECTION_ID_AUTHENTICATE == d_selectionId) {
        d_authenticate.object() = value;
    }
    else {
        reset();
        new (d_authenticate.buffer()) Authenticate(value, d_allocator_p);
        d_selectionId = SELECTION_ID_AUTHENTICATE;
    }
...
#if defined(BSLS_COMPILERFEATURES_SUPPORT_RVALUE_REFERENCES) &&               \
    defined(BSLS_COMPILERFEATURES_SUPPORT_NOEXCEPT)
Authenticate& ControlMessageChoice::makeAuthenticate(Authenticate&& value)
{
    if (SELECTION_ID_AUTHENTICATE == d_selectionId) {
        d_authenticate.object() = bsl::move(value);
    }
    else {
        reset();
        new (d_authenticate.buffer())
            Authenticate(bsl::move(value), d_allocator_p);
        d_selectionId = SELECTION_ID_AUTHENTICATE;
    }
...
AuthenticateResponse& ControlMessageChoice::makeAuthenticateResponse()
{
    ...
    const AuthenticateResponse& value)
{
    ...
    defined(BSLS_COMPILERFEATURES_SUPPORT_NOEXCEPT)
AuthenticateResponse&
ControlMessageChoice::makeAuthenticateResponse(AuthenticateResponse&& value)
{
    if (SELECTION_ID_AUTHENTICATE_RESPONSE == d_selectionId) {
                                          ...
                                          int           spacesPerLevel) const
{
    ...
    case SELECTION_ID_AUTHENTICATE: {
        printer.printAttribute("authenticate", d_authenticate.object());
    } break;
    case SELECTION_ID_AUTHENTICATE_RESPONSE: {
...
const char* ControlMessageChoice::selectionName() const
{
    ...
    case SELECTION_ID_CONFIGURE_STREAM_RESPONSE:
        return SELECTION_INFO_ARRAY[SELECTION_INDEX_CONFIGURE_STREAM_RESPONSE]
            .name();
    case SELECTION_ID_AUTHENTICATE:
        return SELECTION_INFO_ARRAY[SELECTION_INDEX_AUTHENTICATE].name();
    case SELECTION_ID_AUTHENTICATE_RESPONSE:
        return SELECTION_INFO_ARRAY[SELECTION_INDEX_AUTHENTICATE_RESPONSE]
            .name();
    default:
...
ControlMessage::lookupAttributeInfo(const char* name, int nameLength)
{
    ...
    if (bdlb::String::areEqualCaseless("authenticate", name, nameLength)) {
        return &ATTRIBUTE_INFO_ARRAY[ATTRIBUTE_INDEX_CHOICE];
    }

    if (bdlb::String::areEqualCaseless("authenticateResponse",
                                       name,
                                       nameLength)) {
        return &ATTRIBUTE_INFO_ARRAY[ATTRIBUTE_INDEX_CHOICE];
    }
