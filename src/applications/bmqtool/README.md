...
```bash
Usage: bmqtool [--mode <mode>] [-b|broker <address>] [-q|queueuri <uri>]
               [-f|queueflags <flags>] [-l|latency <latency>]
               [--latency-report <report.json>] [-d|dumpmsg] [-c|confirmmsg]
               [-e|eventsize <evtSize>] [-m|msgsize <msgSize>]
               [-r|postrate <rate>] [--eventscount <events>]
               [-u|maxunconfirmed <unconfirmed>] [-i|postinterval <interval>]
               [-v|verbosity <verbosity>] [--logFormat <logFormat>]
               [-D|memorydebug] [-t|threads <threads>]
               [--shutdownGrace <shutdownGrace>] [--nosessioneventhandler]
               [-s|storage <storage>] [--log <log>] [--profile <profile>]
               [-h|help] [--dumpprofile]
