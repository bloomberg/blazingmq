...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    bmqp::PushMessageIterator iter(&bufferFactory, alloc);
    ...
    while (iter.next() == 1) {
        ...
        {
            bdlbb::Blob appDataBlob(&bufferFactory, alloc);
            iter.loadApplicationData(&appDataBlob);
        }

        if (decompressFlag) {
            bdlbb::Blob payloadBlob(&bufferFactory, alloc);
            iter.loadMessagePayload(&payloadBlob);
        }

        {
            bmqu::BlobPosition pos;
            iter.loadApplicationDataPosition(&pos);
        }
    }
