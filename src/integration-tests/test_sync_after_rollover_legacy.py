...
def test_synch_after_missed_rollover(cluster: Cluster, domain_urls: tc.DomainUrls,
    ...
    producer.post(uri_priority, ["msg6"], succeed=True, wait_ack=True)
    producer.post(uri_priority, ["msg7"], succeed=True, wait_ack=True)
    producer.post(uri_priority, ["msg8"], succeed=True, wait_ack=True)
    producer.post(uri_priority, ["msg9"], succeed=True, wait_ack=True)
    # producer.post(uri_priority, ["msg10"], succeed=True, wait_ack=True)
    # producer.post(uri_priority, ["msg11"], succeed=True, wait_ack=True)
    # producer.post(uri_priority, ["msg12"], succeed=True, wait_ack=True)
    # producer.post(uri_priority, ["msg13"], succeed=True, wait_ack=True)
    ...
    # print("leader_journal_file_after_rollover: ", leader_journal_files_after_rollover)
    # print("replica_journal_file_after_rollover: ", replica_journal_files_after_rollover)
    for leader_file, replica_file in zip(sorted(leader_journal_files_after_rollover), sorted(replica_journal_files_after_rollover)):
        ...
        leader_res = subprocess.run(
            [paths.required_paths.storagetool, "--journal-file", leader_file, "--summary"],
            capture_output=True,
            check=True,
        )
        ...
        replica_res = subprocess.run(
            [paths.required_paths.storagetool, "--journal-file", replica_file, "--summary"],
            capture_output=True,
            check=True,
        )
