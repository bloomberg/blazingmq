...
class TestDynamicAppids:
    def test_open_alarm_authorize_post(self, cluster: Cluster, logger):
        ...
        barStatus, bazStatus, fooStatus, quuxStatus = sorted(
            [
                leader.capture(
                    r"(\w+).*: status=(\w+)(?:, StorageIter.atEnd=(\w+))?", 60
                )
    ...
    def test_create_authorize_open_post(self, cluster: Cluster):
        leader = cluster.last_known_leader
        proxies = cluster.proxy_cycle()
    ...
    def test_load_domain_authorize_open_post(self, cluster: Cluster):
        leader = cluster.last_known_leader
        proxies = cluster.proxy_cycle()
    ...
    def _test_authorize_before_domain_loaded(self, cluster):
        leader = cluster.last_known_leader
        proxies = cluster.proxy_cycle()
    ...
    # following test cannot run yet, because domain manager claims domain
    # does not exist if no queue exists in it
    def _test_command_errors(self, cluster):
        proxies = cluster.proxy_cycle()
    ...
    def test_unregister_in_presence_of_queues(self, cluster: Cluster, logger):
        leader = cluster.last_known_leader
        proxies = cluster.proxy_cycle()
    ...
    def test_dynamic_twice_alarm_once(self, cluster: Cluster):
        leader = cluster.last_known_leader
        proxies = cluster.proxy_cycle()
    ...
    @set_max_messages
    def test_unauthorized_appid_doesnt_hold_messages(self, cluster: Cluster):
    ...
    @set_max_messages
    def test_deauthorized_appid_doesnt_hold_messages(self, cluster: Cluster):
        ...
        assert consumer.wait_push_event()
        assert wait_until(
            lambda: len(consumer.list(appid_uri, block=True)) == num_msgs, 3
        )
        msgs = consumer.list(appid_uri, block=True)
        for _ in msgs:
    ...
    def test_unauthorization(self, cluster: Cluster):
    ...
    def test_two_consumers_of_unauthorized_app(self, standard_cluster: Cluster):
