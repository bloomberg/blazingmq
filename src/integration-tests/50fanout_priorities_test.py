...
class TestFanoutPriorities:
    """
    This suite of test cases exercises functionality of prioritizing
    fanout consumers .
    """

    def test_fanout_priorities(self, cluster: Cluster):
        ...
        self._verify_delivery(
            producers, "before", highPriorityQueues, lowPriorityQueues
        )
        ...
        self._verify_delivery(producers, "after", lowPriorityQueues, highPriorityQueues)
    ...
    def _verify_delivery(
        self, producers, message, highPriorityQueues, lowPriorityQueues
    ):
        ...
        for queue in lowPriorityQueues:
            ...
            assert not wait_until(
                lambda: len(queue.list(block=True)) == 1, 0.1, quiet=True
            )
