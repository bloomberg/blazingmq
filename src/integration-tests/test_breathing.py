...
class TestBreathing:
    """This suite of test cases exercises basic routing functionality to
    in the presence of all types of queues.
    """

    def _close_clients(self, clients, uris):
        for client, uri in zip(clients, uris):
            assert client.close(uri, block=True) == Client.e_SUCCESS

    def _stop_clients(self, clients):
        for client in clients:
            assert client.stop_session(block=True) == Client.e_SUCCESS

    def _verify_delivery(self, consumer, uri, messages, timeout=2):
    ...
    def _verify_delivery_and_confirm(self, consumer, uri, messages):
        self._verify_delivery(consumer, uri, messages)
        assert consumer.confirm(uri, "*", block=True) == Client.e_SUCCESS

    def _verify_delivery_and_confirm_balanced(
        self, consumer, uris, messages, timeout=3
    ):
        consumer.wait_push_event()
    ...
    def _verify_max_messages_max_bytes_routing(
        self, producer, consumer, other_consumers
    ):
        ...
        self._verify_delivery(consumer.handle, consumer.uri, ["123"])
        # not confirming

        for anotherConsumer in other_consumers:
            self._verify_delivery_and_confirm(
        ...
        for anotherConsumer in other_consumers:
            self._verify_delivery_and_confirm(
                anotherConsumer.handle, anotherConsumer.uri, ["1"]
            )
        ...
        assert (
            consumer.handle.confirm(consumer.uri, "*", block=True) == Client.e_SUCCESS
        )

        # onHandleUsable kicks in
        self._verify_delivery(consumer.handle, consumer.uri, ["1"])
        # not confirming
        ...
        for anotherConsumer in other_consumers:
            self._verify_delivery_and_confirm(
                anotherConsumer.handle, anotherConsumer.uri, ["2"]
            )

        self._verify_delivery(consumer.handle, consumer.uri, ["1", "2"])
        # not confirming
        ...
        for anotherConsumer in other_consumers:
            self._verify_delivery_and_confirm(
                anotherConsumer.handle, anotherConsumer.uri, ["3"]
            )
        ...
        # onHandleUsable kicks in
        self._verify_delivery(consumer.handle, consumer.uri, ["3"])

    def _verify_priority_routing(self, producers, consumers, lowPriorityConsumers):
        ...
        for producer in producers:
            assert (
                producer.post(
                    tc.URI_PRIORITY, payload=["msg"], block=True, wait_ack=True
                )
                == Client.e_SUCCESS
            )

        for consumer in consumers:
            self._verify_delivery_and_confirm(consumer, tc.URI_PRIORITY, ["msg"])
    ...
    def test_open_queue(self, cartesian_product_cluster: Cluster):
    ...
    def test_verify_priority(self, cluster: Cluster):
        ...
        assert (
            consumer1.open(
                tc.URI_PRIORITY, flags=["read"], consumer_priority=2, block=True
            )
            == Client.e_SUCCESS
        )
        ...
        assert (
            consumer3.open(
                tc.URI_PRIORITY, flags=["read"], consumer_priority=1, block=True
            )
            == Client.e_SUCCESS
        )
        ...
        self._verify_max_messages_max_bytes_routing(
    ...
    def test_verify_fanout(self, cluster: Cluster):
        # 1: Setup producers and consumers
        proxies = cluster.proxy_cycle()
        ...
        assert (
            barConsumerOnPrimaryProxy.open(
                tc.URI_FANOUT_BAR, flags=["read"], block=True
            )
            == Client.e_SUCCESS
        )
        ...
        assert (
            barConsumerOnReplicaProxy.open(
                tc.URI_FANOUT_BAR, flags=["read"], block=True
            )
            == Client.e_SUCCESS
        )
        ...
        assert (
            len(
                fooConsumerAndProducerOnPrimaryProxy.list(tc.URI_FANOUT_FOO, block=True)
            )
            == 0
        )
        ...
        self._verify_delivery_and_confirm(
            barConsumerOnReplicaProxy, tc.URI_FANOUT_BAR, ["msg1"]
        )
        ...
        self._verify_delivery_and_confirm(
            barConsumerOnReplicaProxy, tc.URI_FANOUT_BAR, ["msg2"]
        )
        ...
        self._verify_max_messages_max_bytes_routing(
            ...
            [
                BmqClient(
                    handle=fooConsumerAndProducerOnPrimaryProxy, uri=tc.URI_FANOUT_FOO
                )
            ],
        )
    ...
    def test_verify_broadcast(self, cluster: Cluster):
        # 1: Setup producers and consumers
        proxies = cluster.proxy_cycle()
        ...
        consumer1 = proxy1.create_client("consumer1")
        assert (
            consumer1.open(tc.URI_BROADCAST, flags=["read"], block=True)
            == Client.e_SUCCESS
        )
        ...
        consumer2 = proxy2.create_client("consumer2")
        assert (
            consumer2.open(tc.URI_BROADCAST, flags=["read"], block=True)
            == Client.e_SUCCESS
        )
        ...
        assert (
            producer1.post(
                tc.URI_BROADCAST, payload=["msg1"], block=True, wait_ack=True
            )
            == Client.e_SUCCESS
        )

        self._verify_delivery_and_confirm(consumer1, tc.URI_BROADCAST, ["msg1"])

        self._verify_delivery_and_confirm(consumer2, tc.URI_BROADCAST, ["msg1"])

        assert (
            producer2.post(
                tc.URI_BROADCAST, payload=["msg2"], block=True, wait_ack=True
            )
            == Client.e_SUCCESS
        )

        self._verify_delivery_and_confirm(consumer1, tc.URI_BROADCAST, ["msg2"])

        self._verify_delivery_and_confirm(consumer2, tc.URI_BROADCAST, ["msg2"])

        # 4: Close everything
        self._close_clients(
            [producer1, consumer1, producer2, consumer2], [tc.URI_BROADCAST]
        )

        self._stop_clients([producer1, consumer1, producer2, consumer2])

    def test_verify_redelivery(self, cluster: Cluster):
        ...
        self._stop_clients([producer, consumer1, consumer2])

    def test_verify_priority_queue_redelivery(self, cluster: Cluster):
        ...
        self._stop_clients([producer, consumer])

    def test_verify_partial_close(self, standard_cluster: Cluster):
        ...
        self._stop_clients([producer1, producer2])

    @start_cluster(True, True, True)
    @tweak.cluster.queue_operations.open_timeout_ms(2)
    def test_command_timeout(self, standard_cluster: Cluster):
    ...
    def test_queue_purge_command(self, standard_cluster: Cluster):
        ...
        leader.command(
            f"DOMAINS DOMAIN {tc.DOMAIN_FANOUT} QUEUE {tc.TEST_QUEUE} PURGE foo"
        )
        ...
        leader.command(
            f"DOMAINS DOMAIN {tc.DOMAIN_FANOUT} QUEUE {tc.TEST_QUEUE} PURGE *"
        )
    ...
    def test_message_properties(self, cluster: Cluster):
        ...
        assert (
            consumer.open(tc.URI_PRIORITY, flags=["read"], block=True)
            == Client.e_SUCCESS
        )
        ...
        self._stop_clients([producer1, consumer, producer2])
