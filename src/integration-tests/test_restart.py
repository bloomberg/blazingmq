...
class TestRestartCluster:
    """This suite of test cases exercises basic functionality of the cluster
    while restarting all nodes in the cluster.
    """

    def test_basic(self, cluster: Cluster):
        ...
        assert wait_until(
            lambda: len(consumer.list(tc.URI_PRIORITY, block=True)) == 2, 2
        )

    def test_wrong_domain(self, cluster: Cluster):
        proxies = cluster.proxy_cycle()
        producer = next(proxies).create_client("producer")

        assert Client.e_SUCCESS is producer.open(
            tc.URI_FANOUT, flags=["write"], block=True
        )
    ...
    def test_migrate_domain_to_another_cluster(self, cluster: Cluster):
        proxies = cluster.proxy_cycle()
        producer = next(proxies).create_client("producer")

        assert Client.e_SUCCESS == producer.open(
            tc.URI_FANOUT, flags=["write"], block=True
        )
