...
def broker_authn_config(password: Optional[str] = None) -> dict:
    """
    Return an authentication config where every broker accepts `BROKER_USER`
    with `BROKER_PASSWORD`.  If the specified `password` is set, the broker
    also presents `BROKER_USER` with that password when connecting to another
    broker; if it is omitted, the broker is left without a credential
    provider.
    """
    config = {
        "authenticators": [
...
@tweak.broker.app_config.authentication(broker_authn_config())
def test_broker_to_broker_no_credential_provider(
    multi_node: Cluster,
    sc_domain_urls: tc.DomainUrls,  # pylint: disable=unused-argument
) -> None:
    """
    Test that a broker with no credential provider negotiates its outbound
    connections directly, without authenticating itself to its peers.
    """
    # Reaching this point means a leader was elected, which requires the nodes
    # to have established sessions with one another.
    assert multi_node.last_known_leader is not None
    ...
    for node in multi_node.nodes():
        # Stop at whichever comes first: a session is opened, or an
        # AuthenticationRequest is sent.  Ordering the two makes the absence of
        # the request an observation rather than a timeout.
        matches = node.capture_n(
            ["Sending AuthenticationRequest", r"Open session '.*' for channel"],
            count=1,
        )
