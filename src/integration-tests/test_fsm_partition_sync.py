...
@tweak.cluster.partition_config.grow_step_percent(GROW_STEP_PERCENT)
@tweak.cluster.queue_operations.shutdown_timeout_ms(100)
def test_rollover_with_file_size_decrease(
    fsm_multi_cluster: Cluster,
    domain_urls: tc.DomainUrls,
) -> None:
    """
    Test that during rollover journal file size could be dynamically changed:
    increased by grow_step_percent from maxJournalFileSize and then
    decreased back to maxJournalFileSize.
    - start cluster
    - put some messages to initiate rollover with journal file size increase
    - assert that journal file size is increased at both primary and replica
    - confirm all messages to make space for rollover
    - put some messages with confirm to initiate rollover
