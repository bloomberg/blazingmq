...
from bmq.dev.it.process import proc
import bmq.dev.it.process.bmqproc
import bmq.dev.it.logre
import bmq.dev.it.testconstants as tc
import bmq.dev.workspace as ws

from bmq.dev.it.util import internal_use, ListContextManager, Queue
...
class Broker(bmq.dev.it.process.bmqproc.BMQProcess):
    ...
    def wait_status(
        ...
        if wait_leader:
            ...
            regexes.append(bmq.dev.it.logre.leader)

        if wait_ready:
            self._logger.log(self._log_level, "Waiting for cluster to be ready...")
            regexes.append(bmq.dev.it.logre.cluster_is_available(cluster))
