...
from bmq.dev.it.process.proc import Process, Command
...
from bmq.dev.it.logging import BallLoggerAdapter
...
class Process:
    ...
    def wait(self, timeout: Optional[float] = None) -> int:
        ...
        try:
            ...
            self._logger.warning("timed out while waiting for process to exit")
            return 1

        self._internal_logger.debug("waiting for stdout consuming thread to exit...")
        self._stdout_thread.join()

        self._internal_logger.debug("waiting for stderr consuming thread to exit...")
