...
import bmq.dev.it.logre
import bmq.dev.it.process.proc
import bmq.dev.it.testconstants as tc
import bmq.dev.workspace as ws
import bmq.util.logging as bul
from bmq.dev.it.process.broker import Broker
from bmq.dev.it.process.client import Client
from bmq.dev.it.util import ListContextManager, Queue, internal_use
...
class Cluster(contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        self._logger = bmq.dev.it.process.proc.BallLoggerAdapter(
            logging.getLogger("test"), extra=log_extra
        )

        self.last_known_leader = None
        self.work_dir = location.root_dir
    ...
    def create_client(
        ...
        if proxyhostname and proxy:
            raise RuntimeError("proxyhostname and proxy cannot be both specified")

        if proxyhostname is None:
            if proxy is None:
                raise RuntimeError("either proxyhostname or proxy must be specified")
            proxyhostname = proxy.name
    ...
    def deploy_domains(self):
        self.workspace.deploy_domains(self.location)

    def reconfigure_domain_values(
        self,
        domain: str,
        kvs: Dict[str, Any],
        ...
        for key_path, value in kvs.items():
            path = key_path.split(".")
            leaf = path.pop()
            for broker in self.workspace.brokers.values():
                obj = broker.domains[domain].definition.parameters
                for attr in path:
