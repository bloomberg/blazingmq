...
from bmq.dev.paths import paths
...
class Location(abc.ABC):
    ...
    @abc.abstractmethod
    def install(
        self, broker: Broker, from_path: Union[str, Path], to_path: Union[str, Path]
    ) -> None:
        ...

    @abc.abstractmethod
    def create_file(
        self, broker: Broker, path: Union[str, Path], content: str, *, mode=None
    ) -> None:
        ...
    ...
    @abc.abstractmethod
    def rmdir(self, broker: Broker, path: Union[str, Path]) -> None:
        ...
...
class Workspace:
    ...
    * Write the configuration for each broker in its own directory, and create
    ...
    def deploy_broker_config(
        self, location: Location, brokers: Optional[List[Broker]] = None
    ) -> None:
        for broker in brokers or self.brokers.values():
            location.create_json_file(broker, "etc/bmqbrkrcfg.json", broker.config)

            log_dir = Path(broker.config.task_config.log_controller.file_name).parent  # type: ignore
            if log_dir != Path():
                location.mkdir(broker, log_dir)

            stats_dir = Path(broker.config.app_config.stats.printer.file).parent  # type: ignore
            if stats_dir != Path():
                location.mkdir(broker, stats_dir)

    def deploy_domains(
        self,
        location: Location,
        brokers: Optional[Iterable[Broker]] = None,
    ) -> None:
        for broker in brokers or self.brokers.values():
            if brokers and broker not in brokers:
                continue

            location.create_json_file(broker, "etc/clusters.json", broker.clusters)
            location.rmdir(broker, "etc/domains")
...
class HostLocation(Location):
    ...
    def __init__(self, workspace: Workspace, root_dir: Union[Path, str]):
    ...
    def rmdir(self, broker: Broker, path: Union[str, Path]) -> None:
        rmtree(self.broker_path(broker, path), ignore_errors=True)

    def install(
        self, broker: Broker, from_path: Union[str, Path], to_path: Union[str, Path]
    ) -> None:
        from_path = Path(from_path).resolve()
        to_path = self.broker_path(broker, to_path)
    ...
    def create_file(
        ...
        path = self.broker_path(broker, path)
    ...
    def run(self):
        ...
        with self.Brokers(self) as build:
            for broker in self.workspace.brokers.values():
                ...
                build.brokers[broker] = monitored

                monitored.process = subprocess.Popen(
                    [str(self.broker_path(broker, "run"))],
            ...
            result = self.Brokers(self, build.brokers)
            build.brokers = {}
