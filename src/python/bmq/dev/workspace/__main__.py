"""
Create a workspace for running a cluster.

BMQ workspace generator

usage: python -m bmq.dev.workspace [-h] [--log-level LEVELS] --workdir WORKDIR [--port-base PORT]

optional arguments:
  -h, --help            show this help message and exit
  --log-level LEVELS, -l LEVELS
                        set log level(s) as specified by LEVELS
  --workdir WORKDIR, -w WORKDIR
                        write files in WORKDIR
  --port-base PORT, -b PORT
                        allocate ports starting at PORT (use ephemeral ports if not specified)
"""

import argparse
import itertools
import logging
import signal
import socket
from pathlib import Path

import bmq.util.logging as bul
from bmq.dev.workspace import Workspace, HostLocation, logger

parser = argparse.ArgumentParser(
    prog="python -m bmq.dev.workspace",
    description="BMQ workspace generator",
    parents=[bul.make_parser()],
)

parser.add_argument(
    "--workdir",
    "-w",
    type=Path,
    action="store",
    required=True,
    metavar="WORKDIR",
    help="write files in WORKDIR",
)

parser.add_argument(
    "--port-base",
    "-b",
    type=int,
    action="store",
    required=False,
    metavar="PORT",
    help="allocate ports starting at PORT (use ephemeral ports if not specified)",
)

args = parser.parse_args()

for handler in logging.root.handlers:
    handler.setFormatter(logging.Formatter("%(message)s"))


def ephemeral_port_allocator():
    while True:
        sock = socket.socket()
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        sock.bind(("0.0.0.0", 0))
        yield sock.getsockname()[1]


workspace = Workspace()

if args.port_base is not None:
    port_allocator = itertools.count(args.port_base)
else:
    port_allocator = ephemeral_port_allocator()

cluster = workspace.cluster(
    name="c2x2",
    nodes=[
        workspace.broker(
            name="east/1",
            tcp_host="localhost",
            tcp_port=next(port_allocator),
            data_center="east",
        ),
        workspace.broker(
            name="east/2",
            tcp_host="localhost",
            tcp_port=next(port_allocator),
            data_center="east",
        ),
        workspace.broker(
            name="west/1",
            tcp_host="localhost",
            tcp_port=next(port_allocator),
            data_center="west",
        ),
        workspace.broker(
            name="west/2",
            tcp_host="localhost",
            tcp_port=next(port_allocator),
            data_center="west",
        ),
    ],
)


priority_domain = cluster.priority_domain("bmq.test.mmap.priority")
# TODO: move test domain definitions to this package, add them to the
# workspace.

local = HostLocation(workspace, args.workdir)

local.deploy()


def on_signal(signum: int, frame) -> None:
    """
    Signal handler.

    Do nothing, 'main' just waits for C-C with signal.pause().
    """

    logger.info("received signal: %s. Exiting...", signum)


with local.run() as brokers:
    signal.signal(signal.SIGINT, on_signal)  # handle CTRL-C
    signal.signal(signal.SIGTERM, on_signal)

    for broker in brokers.brokers.keys():
        print("{}: tcp://{}:{}".format(broker.name, broker.host, broker.port))

    print("C-c to exit...")
    signal.pause()
