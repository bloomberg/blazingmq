...
class Configurator:
    ...
    * Write the configuration for each broker in its own directory, and create
        ...
        *,
        ...
        Returns:
            A Broker object representing the fully configured broker
                configuration.
        """
        if (tcp_host is None and tcp_port is None) != (listeners is None):
            raise ValueError(
    ...
    def cluster(
        ...
        tcp://<broker.tcp_host>:<port> where <port> is specified by the special listener
        named `inter_broker_listener`. Otherwise, the first interface specified in the
        broker's listener configuration will be chosen.
        ...
        self._prepare_cluster(name, nodes, definition, inter_broker_listener)
    ...
    def deploy_programs(self, broker: Broker, site: Site) -> None:
        ...
        logger.debug("deploy_programs %s :: %s", broker.name, broker_path)
