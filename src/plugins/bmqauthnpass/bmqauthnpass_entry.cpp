...
void instantiatePluginLibrary(
    ...
    bslma::Allocator*                          allocator)
{
    BALL_LOG_SET_CATEGORY("AUTHNPASS.ENTRY");

    BALL_LOG_INFO << "Instantiating 'libbmqauthnpass.so' plugin library "
                     "(version: "
                  << bmqauthnpass::Version::version() << ")";

    *library =
        bslma::ManagedPtrUtil::allocateManaged<bmqauthnpass::PluginLibrary>(
            allocator);
}
