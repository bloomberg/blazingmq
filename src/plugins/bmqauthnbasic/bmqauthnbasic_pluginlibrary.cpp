...
#include <bmqauthnbasic_pluginlibrary.h>
...
: d_plugins(allocator)
{
    mqbplug::PluginInfo& authnbasicPluginInfo = d_plugins.emplace_back(
        mqbplug::PluginType::e_AUTHENTICATOR,
        "BasicAuthenticator");

    authnbasicPluginInfo.setFactory(
        bsl::allocate_shared<BasicAuthenticatorPluginFactory>(allocator));
    authnbasicPluginInfo.setVersion(Version::version());
    authnbasicPluginInfo.setDescription("Basic Authenticator");
}
