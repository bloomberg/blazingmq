...
#ifndef INCLUDED_AUTHNBASIC_VERSIONTAG
#define INCLUDED_AUTHNBASIC_VERSIONTAG

//@PURPOSE: Provide versioning information for the 'bmqauthnbasic' plugin.
//
//@SEE_ALSO: bmqauthnbasic::Version
//
//@DESCRIPTION: This component provides versioning information for the
// 'bmqauthnbasic' plugin.  The 'AUTHNBASIC_VERSION' macro that is supplied can
// be used for conditional-compilation based on 'bmqauthnbasic' version
// information. The following usage example illustrates this basic capability.
//
/// Usage
///-----
// At compile time, the version of AUTHNBASIC can be used to select an older or
...
// following, the '#if' preprocessor directive compares 'AUTHNBASIC_VERSION'
// (i.e., the latest AUTHNBASIC version, excluding the patch version) to a
// specified major and minor version composed using the 'BDE_MAKE_VERSION'
// macro:
//..
//  #if AUTHNBASIC_VERSION > BDE_MAKE_VERSION(1, 3)
//      // Call 'newFunction' for AUTHNBASIC versions later than 1.3.
//      int result = newFunction();
//  #else
//      // Call 'oldFunction' for AUTHNBASIC version 1.3 or earlier.
//      int result = oldFunction();
//  #endif
//..

/// AUTHNBASIC release major version
#define AUTHNBASIC_VERSION_MAJOR 99

/// AUTHNBASIC release minor version
#define AUTHNBASIC_VERSION_MINOR 99

/// AUTHNBASIC patch level
#define AUTHNBASIC_VERSION_PATCH 99
...
#define AUTHNBASIC_MAKE_VERSION(major, minor) ((major) * 10000 + (minor) * 100)

/// Similar to AUTHNBASIC_MAKE_VERSION(), but include patch number as well.
#define AUTHNBASIC_MAKE_EXT_VERSION(major, minor, patch)                      \
    ((major) * 10000 + (minor) * 100 + (patch))

/// Construct a composite version number in the range [ 0 .. 999900 ] from
/// the specified 'AUTHNBASIC_VERSION_MAJOR' and 'AUTHNBASIC_VERSION_MINOR'
/// numbers corresponding to the major and minor version numbers,
/// respectively, of the current (latest) AUTHNBASIC release.  Note that the
/// patch version number is intentionally not included.  For example,
/// 'AUTHNBASIC_VERSION' produces 10300 (decimal) for AUTHNBASIC version
/// 1.3.1.
#define AUTHNBASIC_VERSION                                                    \
    AUTHNBASIC_MAKE_VERSION(AUTHNBASIC_VERSION_MAJOR, AUTHNBASIC_VERSION_MINOR)

/// Similar to AUTHNBASIC_VERSION, but include the patch number as well
#define AUTHNBASIC_EXT_VERSION                                                \
    AUTHNBASIC_MAKE_EXT_VERSION(AUTHNBASIC_VERSION_MAJOR,                     \
                                AUTHNBASIC_VERSION_MINOR,                     \
                                AUTHNBASIC_VERSION_PATCH)
