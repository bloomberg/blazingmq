...
main() {
  ...
  if [[ -z "${CODEGEN+}" ]]; then
    cat <<END >&2
This script is not able to be run by non-Bloomberg developers as it relies on
...
script with the CODEGEN enviornment variable set to the internal codegen tool.
END
    exit 1
  fi
